                        ...
                        uint8_t *iv_out)
{
    ...
    hdr->type = AESCRYPT_TYPE_ENCRYPTED;
    hdr->crc  = 0; /* filled after encryption */
                        ...
                        const char *arg4, int has_arg5)
{
    ...
    /*
     * Key derivation: try HW_KMC_CfgGetKey (embedded fallback when no
     * kmc_store is available).  On device: key from eFuse + PBKDF2.
     */
    memset(key, 0, sizeof(key));
    extern int HW_KMC_CfgGetKey(const char *, void *, size_t);
    HW_KMC_CfgGetKey("/var/new_key_encryte_ctree", key, sizeof(key));
                        ...
                        const char *arg4, int has_arg5)
{
    ...
    uint8_t key[32]; /* AES-256 */
    uint8_t iv[AESCRYPT_IV_LEN];
    uint8_t iv_save[AESCRYPT_IV_LEN];
    mbedtls_aes_context aes;
    FILE *fin = NULL, *fout = NULL;
    uint8_t block[16];
    size_t n;
    long file_size, ciphertext_len;

    (void)unused; (void)arg4; (void)has_arg5;

    fin = fopen(infile, "rb");
    if (!fin) { ret = 1; goto out; }

    /* Read header: type(4) + CRC(4) + IV(16) */
    if (fread(&hdr, 1, AESCRYPT_HEADER_LEN, fin) != AESCRYPT_HEADER_LEN) {
        ret = 1;
        goto out;
    }
    if (hdr.type != AESCRYPT_TYPE_ENCRYPTED &&
        hdr.type != AESCRYPT_TYPE_SELFENC) {
        HW_OS_Printf("OS_AescryptDecrypt: unsupported type=0x%x\n", hdr.type);
        ret = 1;
        goto out;
    }
    memcpy(iv_save, hdr.iv, AESCRYPT_IV_LEN);

    /* Compute ciphertext length: file_size − 24-byte header, round to 16 */
    fseek(fin, 0, SEEK_END);
    file_size = ftell(fin);
    ciphertext_len = file_size - AESCRYPT_HEADER_LEN;
    ciphertext_len -= ciphertext_len % 16;
    fseek(fin, AESCRYPT_HEADER_LEN, SEEK_SET);

    /* Try auto-detecting the correct key by decrypting the first block
     * and checking for gzip magic (0x1f 0x8b). If AESCRYPT2_KEY_INDEX
     * env is set, use that key directly. */
    {
        extern int  HW_KMC_CfgGetKey(const char *, void *, size_t);
        ...
        int found_key = -1;
        uint8_t first_block[16], dec_block[16];

        if (env && *env >= '0' && *env <= '5') {
            start_idx = end_idx = *env - '0';
        }

        /* Read first ciphertext block for key probe */
        if (fread(first_block, 1, 16, fin) != 16) { ret = 1; goto out; }
        fseek(fin, AESCRYPT_HEADER_LEN, SEEK_SET);

        for (int ki = start_idx; ki <= end_idx; ki++) {
            HW_KMC_SetKeyIndex(ki);
            HW_KMC_CfgGetKey(NULL, key, sizeof(key));
            memcpy(iv, iv_save, AESCRYPT_IV_LEN);

            mbedtls_aes_init(&aes);
            mbedtls_aes_setkey_dec(&aes, key, 256);
            mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_DECRYPT, 16, iv,
                                  first_block, dec_block);
            mbedtls_aes_free(&aes);

            if (dec_block[0] == 0x1f && dec_block[1] == 0x8b) {
                found_key = ki;
                break;
            }
        }
        ...
        if (found_key < 0) {
            HW_OS_Printf("OS_AescryptDecrypt: no matching key found "
                         "(tried %d–%d)\n", start_idx, end_idx);
            ret = 1;
            goto out;
        }

        /* Use the correct key for full decryption */
        HW_KMC_SetKeyIndex(found_key);
        HW_KMC_CfgGetKey(NULL, key, sizeof(key));
    }

    fout = fopen(outfile, "wb");
    if (!fout) { ret = 1; goto out; }
