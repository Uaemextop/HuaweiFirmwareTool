...
do_update_preloader(){
	...
	tar -xf $2 $b_img 2> /dev/null
	tar -xf $2 $nandwritebinary 2> /dev/null
	if [ ! -f "$b_img" ]; then
		return
	fi

	if [ -f "./nandwrite" ]; then
		echo "nandwrite binary is present"
	...
	cat $b_img | md5sum | sed 's/-/'$b_img'/g' > $md5_rt_result
	# Check integrity
	grep $b_img $md5_cmp > $md5_tmp
	diff $md5_rt_result $md5_tmp
	if [ $? != 0 ]; then
			echo "$b_img""md5_sum inconsistent, aborted image updating !"
			rm $b_img
			return 1
	fi
