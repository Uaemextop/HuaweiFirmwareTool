<%SendWebHeadStr(); %>
<title><% multilang("4" "LANG_DEVICE"); %> <% multilang("752" "LANG_IP_RANGE"); %><% multilang("1159" "LANG_TABLE_2"); %></title>
<SCRIPT>
var pool_ipprefix;
...
function addClick(obj)
{
 ...
 if (document.ipRange.Opt60.value=="") {
  alert('<% multilang("2124" "LANG_OPTION60_CANNOT_BE_NULL"); %>');
  document.ipRange.Opt60.focus();
  return false;
 }
 if ( !checkInputIP(document.ipRange.sIp.value ) ) {
  alert('<% multilang("2125" "LANG_INVALID_SOURCE_RANGE_OF_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE"); %>');
  document.ipRange.sIp.focus();
  return false;
 }
 if ( !checkInputIP(document.ipRange.dIp.value ) ) {
  alert('<% multilang("2126" "LANG_INVALID_DESTINATION_RANGE_OF_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE"); %>');
  document.ipRange.dIp.focus();
  return false;
 }
 if(document.ipRange.dhcpDeviceType.selectedIndex==3) {
  if (document.ipRange.stbdns1.value == "") {
   alert('<% multilang("2127" "LANG_ENTER_STBS_DNS_VALUE"); %>');
  ...
  if (document.ipRange.opchaddr.value == "") {
   alert('<% multilang("2128" "LANG_ENTER_STBS_OPCH_ADDRESS_VALUE"); %>');
  ...
  if (document.ipRange.opchport.value == "") {
   alert('<% multilang("2129" "LANG_ENTER_STBS_OPCH_PORT_VALUE"); %>');
   document.ipRange.opchport.focus();
   return false;
  }
  if ( !checkDigitRange(document.ipRange.opchport.value,1,1,65535) ) {
   alert('<% multilang("2130" "LANG_INVALID_OPCH_PORT_IT_SHOULD_BE_1_65535"); %>');
   document.ipRange.opchport.focus();
   return false;
  }
 }
 else if(document.ipRange.dhcpDeviceType.selectedIndex!=5)
 {
  if(document.ipRange.OptStr.value == "") {
   alert('<% multilang("2131" "LANG_ENTER_OPTION_STRING_VALUE"); %>');
   document.ipRange.OptStr.focus();
   return false;
  }
 }
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("4" "LANG_DEVICE"); %> <% multilang("752" "LANG_IP_RANGE"); %><% multilang("1159" "LANG_TABLE_2"); %></p>
...
<form action=/boaform/formIpRange method=POST name="ipRange">
 ...
 <div class="data_common data_common_notitle">
  <table>
   <tr>
    <th width=30%><% multilang("4" "LANG_DEVICE"); %><% multilang("700" "LANG_NAME"); %>:&nbsp;&nbsp;</th>
   ...
   <tr>
    <th><% multilang("367" "LANG_GATEWAY_ADDRESS"); %>:&nbsp;&nbsp;</th>
...
<div class="btn_ctl">
 ...
 <input class="link_bg" type="submit" value="<% multilang("315" "LANG_DELETE"); %>" name="del" onClick="return delClick()">&nbsp;&nbsp;
 <input class="link_bg" type="submit" value="<% multilang("338" "LANG_MODIFY"); %>" name="mod" onClick="return addClick(this)">&nbsp;&nbsp
...
<div class="column">
 <div class="column_title">
  <div class="column_title_left"></div>
   <p><% multilang("4" "LANG_DEVICE"); %> <% multilang("752" "LANG_IP_RANGE"); %><% multilang("1159" "LANG_TABLE_2"); %></p>
