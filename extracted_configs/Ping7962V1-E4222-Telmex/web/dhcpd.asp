<% SendWebHeadStr();%>
<title>DHCP <% multilang("383" "LANG_SETTINGS"); %></title>
<SCRIPT>
var pool_ipprefix;
var initialDhcp;
...
function showDhcpSvr(ip)
{
 ...
 if (document.dhcpd.dhcpdenable[0].checked == true)
  document.getElementById('displayDhcpSvr').innerHTML=
   '<div class="btn_ctl">'+
   '<input type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="save" class="link_bg" onClick="return saveClick(0)">&nbsp;&nbsp;'+
   '</div>';
  ...
  document.getElementById('displayDhcpSvr').innerHTML=
   ...
   '<table>'+
   '<tr><td colspan=2>'+
   '<% multilang("359" "LANG_PAGE_DESC_CONFIGURE_DHCP_SERVER_IP_ADDRESS"); %>'+
   '</td></tr>'+
   '<tr>'+
   ...
   '</table></div>'+
   '<div class="btn_ctl">'+
   '<input type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="save" onClick="return saveClick(1)" class="link_bg">&nbsp;&nbsp;'+
   '</div>';
  ...
  html=
   ...
   '<table>'+
   '<tr><td colspan=2>'+
   '<% multilang("360" "LANG_PAGE_DESC_ENABLE_DHCP_SERVER"); %>'+
   '</td></tr>'+
  ...
  html+=
    '<input class="inner_btn" type="button" value="<% multilang("363" "LANG_SHOW_CLIENT"); %>" name="dhcpClientTbl" onClick="dhcpTblClick(\'/dhcptbl.asp\')" class="inner_btn">'+
    '</td>'+
    '</tr>';
  if (!pool_ipprefix)
  {
  ...
  html += '<tr>'+
   '<th width="30%"><% multilang("364" "LANG_MAX_LEASE_TIME"); %>:</th>'+
   '<td width="70%">'+
   '<input type="text" name="ltime" size=10 maxlength=9 value="<% getInfo("lan-dhcpLTime"); %>"> <% multilang("365" "LANG_SECONDS"); %> (<% multilang("366" "LANG_MINUS_1_INDICATES_AN_INFINITE_LEASE"); %>)'+
   '</td>'+
   '</tr>'+
   '<tr>'+
   '<th width="30%"><% multilang("427" "LANG_DOMAIN"); %><% multilang("700" "LANG_NAME"); %>:</th>'+
   '<td width="70%">'+
   '<input type="text" name="dname" size=32 maxlength=29 value="<% getInfo("lan-dhcpDName"); %>">'+
   '</td>'+
   '</tr>'+
   '<tr>'+
   '<th width="30%"><% multilang("367" "LANG_GATEWAY_ADDRESS"); %>:</th>'+
  ...
  if (en_dnsopt == 0)
   ...
   html +=
    '<table><tr>'+
    '<th width="30%"><% multilang("372" "LANG_DNS_OPTION"); %>:</th>'+
    '<td width=70%><input type=radio name=dhcpdns value=0 onClick=showdns()><% multilang("373" "LANG_USE_DNS_PROXY"); %>&nbsp;&nbsp;'+
    '<input type=radio name=dhcpdns value=1 onClick=showdns()><% multilang("374" "LANG_SET_MANUALLY"); %>&nbsp;&nbsp;</td>'+
    '</tr></table></div>'+
    ...
    '</div>'+
    '<div class="btn_ctl">'+
    '<input type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="save" onClick="return saveChanges()" class="link_bg">&nbsp;&nbsp;'+
    '<input type="button" value="<% multilang("369" "LANG_PORT_BASED_FILTER"); %>" name="macIpTbl" onClick="macIpClick(\'/portBaseFilterDhcp.asp\')" class="link_bg">'+
    '<input type="button" value="<% multilang("368" "LANG_MAC_BASED_ASSIGNMENT"); %>" name="macIpTbl" onClick="macIpClick(\'/macIptbl.asp\')" class="link_bg">'+
    '</div>';
...
function checkLastOctet(ip, gw)
{
   if (ip==document.dhcpd.dhcpRangeStart.value)
   {
       d1 = getDigit(ip,4);
       d2 = getDigit(gw,4);
       d3 = getDigit(document.dhcpd.dhcpRangeEnd.value,4);
       if (d1>d2 && d1<d3)
           return true;
   }
   else if (ip==document.dhcpd.dhcpRangeEnd.value)
   {
       d1 = getDigit(ip,4);
       d2 = getDigit(gw,4);
       d3 = getDigit(document.dhcpd.dhcpRangeStart.value,4);
       if (d1>d2 && d1>d3)
           return true;
   }
   return false;
}
function checkPoolRange(ip, gw)
{
   if (!checkOctet(ip.value, 1, gw.value)) {
                alert("<% multilang("1742" "LANG_INVALID_IPV4_ADDR_1ST_DIGIT_CHECK_GW"); %>");
                ip.value = ip.defaultValue;
                ip.focus();
                return false;
   }
   if (!checkOctet(ip.value, 2, gw.value)) {
                alert("<% multilang("1743" "LANG_INVALID_IPV4_ADDR_2ND_DIGIT_CHECK_GW"); %>");
                ip.value = ip.defaultValue;
                ip.focus();
                return false;
   }
   if (!checkOctet(ip.value, 3, gw.value)) {
                alert("<% multilang("1744" "LANG_INVALID_IPV4_ADDR_3RD_DIGIT_CHECK_GW"); %>");
                ip.value = ip.defaultValue;
                ip.focus();
                return false;
   }
   if (!checkLastOctet(ip.value, gw.value)) {
                alert("<% multilang("1745" "LANG_INVALID_IPV4_ADDR_4TH_DIGIT_CHECK_GW"); %>");
                ip.value = ip.defaultValue;
                ip.focus();
                return false;
   }
...
function saveChanges()
{
  ...
  alert('<% multilang("2132" "LANG_INVALID_DOMAIN_NAME"); %>');
 ...
 if (document.dhcpd.dhcpRangeStart.value=="") {
  alert('<% multilang("2133" "LANG_PLEASE_INPUT_DHCP_IP_POOL_RANGE_"); %>');
 ...
 if ( validateKey( document.dhcpd.dhcpRangeStart.value ) == 0 ) {
  alert('<% multilang("2134" "LANG_INVALID_DHCP_CLIENT_START_RANGE_IT_SHOULD_BE_1_254"); %>');
  document.dhcpd.dhcpRangeStart.value = document.dhcpd.dhcpRangeStart.defaultValue;
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 ...
 if ( !checkSubnet(document.dhcpd.lan_ip.value,document.dhcpd.lan_mask.value,document.dhcpd.dhcpRangeStart.value)) {
  alert('<% multilang("2135" "LANG_INVALID_DHCP_CLIENT_START_ADDRESSIT_SHOULD_BE_LOCATED_IN_THE_SAME_SUBNET_OF_CURRENT_IP_ADDRESS"); %>');
  document.dhcpd.dhcpRangeStart.value = document.dhcpd.dhcpRangeStart.defaultValue;
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 if (document.dhcpd.dhcpRangeEnd.value=="") {
  alert('<% multilang("2136" "LANG_PLEASE_INPUT_DHCP_IP_POOL_RANGE"); %>');
 ...
 if ( validateKey( document.dhcpd.dhcpRangeEnd.value ) == 0 ) {
  alert('<% multilang("2137" "LANG_INVALID_DHCP_CLIENT_END_ADDRESS_RANGE_IT_SHOULD_BE_1_254"); %>');
  document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
  document.dhcpd.dhcpRangeEnd.focus();
  return false;
 }
 if ( !checkDigitRange(document.dhcpd.dhcpRangeEnd.value,1,1,254) ) {
  alert('<% multilang("2138" "LANG_INVALID_DHCP_CLIENT_END_RANGE_IT_SHOULD_BE_1_254"); %>');
  document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
  document.dhcpd.dhcpRangeEnd.focus();
  return false;
 }
 if ( !checkSubnet(document.dhcpd.lan_ip.value,document.dhcpd.lan_mask.value,document.dhcpd.dhcpRangeEnd.value)) {
  alert('<% multilang("2139" "LANG_INVALID_DHCP_CLIENT_END_ADDRESSIT_SHOULD_BE_LOCATED_IN_THE_SAME_SUBNET_OF_CURRENT_IP_ADDRESS"); %>');
  document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
  document.dhcpd.dhcpRangeEnd.focus();
  return false;
 }
 if ( parseInt(document.dhcpd.dhcpRangeStart.value, 10) >= parseInt(document.dhcpd.dhcpRangeEnd.value, 10) ) {
  alert('<% multilang("2140" "LANG_INVALID_DHCP_CLIENT_ADDRESS_RANGEENDING_ADDRESS_SHOULD_BE_GREATER_THAN_STARTING_ADDRESS"); %>');
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 }
  ...
  if (!checkPoolRange(document.dhcpd.dhcpRangeStart, document.dhcpd.ip)) {
                        document.dhcpd.dhcpRangeStart.value = document.dhcpd.dhcpRangeStart.defaultValue;
                        document.dhcpd.dhcpRangeStart.focus();
                        return false;
                }
  ...
  if (!checkPoolRange(document.dhcpd.dhcpRangeEnd, document.dhcpd.ip)) {
                        document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
                        document.dhcpd.dhcpRangeEnd.focus();
                        return false;
                }
 }
 if (!checkInputIP(document.dhcpd.ip.value)) {
  alert('<% multilang("2141" "LANG_INVALID_IP_POOL_RANGE_LAN_IP_MUST_BE_EXCLUDED_FROM_DHCP_IP_POOL"); %>');
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 if ( document.dhcpd.ltime.value=="") {
  alert('<% multilang("2142" "LANG_PLEASE_INPUT_DHCP_LEASE_TIME"); %>');
  document.dhcpd.ltime.focus();
  return false;
 }
 if ( validateKey_leasetime( document.dhcpd.ltime.value ) == 0 ) {
  alert('<% multilang("2143" "LANG_INVALID_DHCP_SERVER_LEASE_TIME_NUMBER"); %>');
  document.dhcpd.ltime.value = document.dhcpd.ltime.defaultValue;
  document.dhcpd.ltime.focus();
  return false;
 }
 if ( !checkDigitRange_leaseTime(document.dhcpd.ltime.value, -1) ) {
  alert('<% multilang("2144" "LANG_INVALID_DHCP_SERVER_LEASE_TIME"); %>');
  document.dhcpd.ltime.value = document.dhcpd.ltime.defaultValue;
  document.dhcpd.ltime.focus();
  return false;
 }
 if (!checkHostIP(document.dhcpd.ip, 1))
  ...
  if (document.dhcpd.dns1.value=="") {
   alert('<% multilang("2152" "LANG_ENTER_DNS_VALUE"); %>');
...
<div class="intro_main ">
 <p class="intro_title">DHCP <% multilang("383" "LANG_SETTINGS"); %></p>
 <p class="intro_content"><% multilang("357" "LANG_PAGE_DESC_CONFIGURE_DHCP_SERVER_RELAY"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th>DHCP <% multilang("143" "LANG_MODE"); %>:</th>
   <td>
   <% checkWrite("dhcpMode"); %>
   </td>
  </tr>
