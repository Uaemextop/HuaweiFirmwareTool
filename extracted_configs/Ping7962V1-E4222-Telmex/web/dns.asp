<%SendWebHeadStr(); %>
<title>DNS <% multilang("245" "LANG_CONFIGURATION"); %></title>
<SCRIPT>
var showIPv6 = 0;
...
function saveClick()
{
 ...
 if(sji_checkip(document.dns.dns1.value) == false){
  ...
  alert("<% multilang("2185" "LANG_IP_ADDRESS_IS_INVALID"); %>");
  return;
 }
 if (document.dns.dns2.value=="") {
  if (document.dns.dns3.value=="") {
   ...
   alert("<% multilang("2186" "LANG_THE_SECOND_DNSV4_NAME_SERVER_S_IP_IS_EMPTY"); %>");
...
function saveClickIpv6()
{
 if (document.dns.dnsv61.value=="") {
  ...
  alert("<% multilang("2187" "LANG_THE_FIRST_DNSV6_NAME_SERVER_S_IP_IS_EMPTY"); %>");
  return false;
 }
 if(document.dns.dnsv61.value != "" ){
  if (! isUnicastIpv6Address( document.dns.dnsv61.value) ){
   alert("<% multilang("2188" "LANG_INVALID_THE_FIRST_DNSV6_NAME_SERVER_S_IP"); %>");
   document.dns.dnsv61.focus();
   return false;
  }
 }
 if (document.dns.dnsv62.value=="") {
  if (document.dns.dnsv63.value=="") {
   ...
   alert("<% multilang("2189" "LANG_THE_SECOND_DNSV6_NAME_SERVER_S_IP_IS_EMPTY"); %>");
 ...
 if(document.dns.dnsv62.value != "" ){
  if (! isUnicastIpv6Address( document.dns.dnsv62.value) ){
   alert("<% multilang("2190" "LANG_INVALID_THE_SECOND_DNSV6_NAME_SERVER_S_IP"); %>");
   document.dns.dnsv62.focus();
   return false;
  }
 }
 if(document.dns.dnsv63.value != "" ){
  if (! isUnicastIpv6Address( document.dns.dnsv63.value) ){
   alert("<% multilang("2191" "LANG_THE_THIRD_DNSV6_NAME_SERVER_S_IP_IS_EMPTY"); %>");
 ...
 if(document.dns.enableDnsBind.checked)
  {
   if(<% checkWrite("WanPvcRouter"); %>) {
    alert("<% multilang("2192" "LANG_NO_ROUTER_WAN_INTERFACE"); %>");
...
<div class="intro_main ">
 <p class="intro_title">DNS <% multilang("245" "LANG_CONFIGURATION"); %></p>
 <p class="intro_content"> <% multilang("375" "LANG_PAGE_DESC_CONFIGURE_DNS_SERVER_IP_ADDRESSES"); %></p>
</div>
 ...
 <table>
  <tr>
   <td width="40%" colspan="2">
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="dnsAuto" name="dnsMode" <% checkWrite("dns0"); %> onClick="autoDNSclicked()"><% multilang("376" "LANG_ATTAIN_DNS_AUTOMATICALLY"); %>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="dnsManual" name="dnsMode" <% checkWrite("dns1"); %> onClick="manualDNSclicked()"><% multilang("377" "LANG_SET_DNS_MANUALLY"); %>
   </td>
  </tr>
