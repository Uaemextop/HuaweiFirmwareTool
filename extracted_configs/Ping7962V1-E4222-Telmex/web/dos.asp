<%SendWebHeadStr(); %>
<title>DoS<% multilang("245" "LANG_CONFIGURATION"); %></title>
<script>
var dosEnabled=0 ;
 ...
 if(rg_config=="yes"){
  if(document.DosCfg.sysfloodSYN.checked){
   if(document.DosCfg.sysfloodSYNcount.value < 5){
    alert('<% multilang("2196" "LANG_MUST_LARGER_THAN_5_PACKETS_SECOND"); %>');
    return false;
   }
  }
...
<div class="intro_main ">
 <p class="intro_title">DoS <% multilang("245" "LANG_CONFIGURATION"); %></p>
 <p class="intro_content"> <% multilang("876" "LANG_DOS_DENIAL_OF_SERVICE_ATTACK_WHICH_IS_LAUNCHED_BY_HACKER_AIMS_TO_PREVENT_LEGAL_USER_FROM_TAKING_NORMAL_SERVICES_IN_THIS_PAGE_YOU_CAN_CONFIGURE_TO_PREVENT_SOME_KINDS_OF_DOS_ATTACK"); %></p>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width="30%">
    <input type="checkbox" name="dosEnabled" value="ON" onclick="dosEnabledClick()"> <% multilang("877" "LANG_ENABLE_DOS_BLOCK"); %>
   </th>
  </tr>
 </table>
</div>
<div class="data_common data_common_notitle">
 <table>
  <tr>
   ...
   <td width="50%"><input type="text" name="sysfloodSYNcount" size="6" maxlength="4" value=<% getInfo("syssynFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  </tr>
  <tr>
   <th><input type="checkbox" name="sysfloodFIN" value="ON" > Whole System Flood: FIN </th>
   <td><input type="text" name="sysfloodFINcount" size="6" maxlength="4" value=<% getInfo("sysfinFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  </tr>
  <tr>
   <th><input type="checkbox" name="sysfloodUDP" value="ON" > Whole System Flood: UDP </th>
   <td><input type="text" name="sysfloodUDPcount" size="6" maxlength="4" value=<% getInfo("sysudpFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  <tr>
   <th><input type="checkbox" name="sysfloodICMP" value="ON" > Whole System Flood: ICMP </th>
   <td><input type="text" name="sysfloodICMPcount" size="6" maxlength="4" value=<% getInfo("sysicmpFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%><br></td>
  </tr>
  <tr>
   <th><input type="checkbox" name="ipfloodSYN" value="ON" > Per-Source IP Flood: SYN </th>
   <td><input type="text" name="ipfloodSYNcount" size="6" maxlength="4" value=<% getInfo("pipsynFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  </tr>
  <tr>
   <th><input type="checkbox" name="ipfloodFIN" value="ON" > Per-Source IP Flood: FIN </th>
   <td><input type="text" name="ipfloodFINcount" size="6" maxlength="4" value=<% getInfo("pipfinFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  </tr>
  <tr>
   <th><input type="checkbox" name="ipfloodUDP" value="ON" > Per-Source IP Flood: UDP </th>
   <td><input type="text" name="ipfloodUDPcount" size="6" maxlength="4" value=<% getInfo("pipudpFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  </tr>
  <tr>
   <th><input type="checkbox" name="ipfloodICMP" value="ON" > Per-Source IP Flood: ICMP </th>
   <td><input type="text" name="ipfloodICMPcount" size="6" maxlength="4" value=<% getInfo("pipicmpFlood"); %> ><% multilang("3013" "LANG_PACKETS_PER_SECOND");%></td>
  </tr>
  <tr>
   ...
   <td>
    ...
    <% multilang("3014" "LANG_PORTSCAN_SENSITIVITY"); %>
   </td>
  </tr>
  <tr>
...
<div class="btn_ctl clearfix">
    <input class="link_bg" type="button" value="<% multilang("878" "LANG_SELECT_ALL"); %>" name="selectAll" onClick="SelectAll()">&nbsp;&nbsp;
    <input class="link_bg" type="button" value="<% multilang("879" "LANG_CLEAR"); %>" name="clearAll" onClick="ClearAll()">&nbsp;&nbsp;
</div>
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width="50%"><input type="checkbox" name="sourceIPblock" value="ON" > <% multilang("880" "LANG_ENABLE_SOURCE_IP_BLOCKING"); %> </th>
   <td width="50%"><input type="text" name="IPblockTime" size="4" maxlength="3" value=<% getInfo("blockTime"); %> > <% multilang("881" "LANG_BLOCK_INTERVAL"); %> (<% multilang("365" "LANG_SECONDS"); %>)</td>
  </tr>
 </table>
</div>
<div class="btn_ctl clearfix">
