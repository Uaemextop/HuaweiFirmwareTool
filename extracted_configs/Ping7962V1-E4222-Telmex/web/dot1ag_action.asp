<%SendWebHeadStr(); %>
<title><% multilang("1199" "LANG_CFM_802_1AG_ACTION"); %></title>
...
function update_ui()
{
 with(document.dot1agAction)
 {
  if(processing == true)
  {
   document.getElementById('status').innerHTML = "<b><% multilang("3" "LANG_STATUS"); %>:</b> <% multilang("1200" "LANG_PROCESSING"); %>";
   start.disabled = true;
  }
  else
  {
   document.getElementById('status').innerHTML = "<b><% multilang("3" "LANG_STATUS"); %>:</b> <% multilang("1201" "LANG_READY"); %>";
...
function startClick()
{
 ...
 if(document.dot1agAction.action[0].checked == true)
 {
  ...
  {
   alert('<% multilang("2199" "LANG_COUNT_CANNOT_BE_EMPTY"); %>');
   document.dot1agAction.count.focus();
   return false;
  }
  if (checkDigit(document.dot1agAction.count.value) == 0)
  {
   alert('<% multilang("2200" "LANG_COUNT_SHOULD_BE_A_POSITIVE_NUMBER"); %>');
   document.dot1agAction.count.focus();
   document.dot1agAction.count.focus();
   return false;
  }
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("1199" "LANG_CFM_802_1AG_ACTION"); %></p>
</div>
<form action=/boaform/formDot1agAction method=POST name="dot1agAction">
<font size=2><b><% multilang("1202" "LANG_PLEASE_SELECT_ONE_OF_FOLLOWING_ENTRY"); %>:</b></font>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width=30%><% multilang("710" "LANG_ACTION"); %>:&nbsp;&nbsp;</th>
   <td><input type="radio" name="action" value="lbm" onClick="update_ui()"><font size=2> <% multilang("1206" "LANG_SEND_LBM"); %>&nbsp;&nbsp;
    <input type="radio" name="action" value="ltm" onClick="update_ui()"><font size=2> <% multilang("1207" "LANG_SEND_LTM"); %>
   </td>
  </tr>
  <tr id="tr_multicast">
   <th><% multilang("1208" "LANG_USE_MULTICAST"); %>:</th>
   <td><input type="checkbox" name="multicast" value=1 onClick="update_ui()"></td>
  </tr>
  <tr>
      <th width=30%><% multilang("1209" "LANG_DESTINATION_MAC_ADDRESS"); %>:&nbsp;&nbsp;</th>
      <td><input type="text" name="mac" size="20" maxlength="17"></td>
  </tr>
  <tr id="tr_count">
      <th width=30%><% multilang("1210" "LANG_LBM_COUNT"); %>:&nbsp;&nbsp;</th>
      <td><input type="text" name="count" size="20" maxlength="5"></td>
  </tr>
 </table>
</div>
<div class="btn_ctl">
<input class="link_bg" type="button" value="<% multilang("517" "LANG_START"); %>" name="start" onClick="startClick()">&nbsp;&nbsp;
</div>
<div id="status"></div>
<div class="column">
 <div class="column_title">
  <div class="column_title_left"></div>
   <p><% multilang("1211" "LANG_RESULT"); %></p>
