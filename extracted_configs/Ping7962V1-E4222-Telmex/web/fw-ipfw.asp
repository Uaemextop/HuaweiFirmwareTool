...
function addClick(obj)
{
 ...
 if (document.formIPFwAdd.l_ip.value=="") {
  alert('<% multilang("2217" "LANG_EMPTY_LOCAL_IP_ADDRESS"); %>');
  document.formIPFwAdd.l_ip.focus();
  return false;
 }
 if ( validateKey( document.formIPFwAdd.l_ip.value ) == 0 ) {
  alert('<% multilang("2218" "LANG_INVALID_LOCAL_IP_ADDRESS_VALUE_"); %>');
  document.formIPFwAdd.l_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.l_ip.value,1,0,255) ) {
  alert('<% multilang("2219" "LANG_INVALID_LOCAL_IP_ADDRESS_RANGE_IN_1ST_DIGIT"); %>');
  document.formIPFwAdd.l_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.l_ip.value,2,0,255) ) {
  alert('<% multilang("2220" "LANG_INVALID_LOCAL_IP_ADDRESS_RANGE_IN_2ND_DIGIT"); %>');
  document.formIPFwAdd.l_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.l_ip.value,3,0,255) ) {
  alert('<% multilang("2221" "LANG_INVALID_LOCAL_IP_ADDRESS_RANGE_IN_3RD_DIGIT"); %>');
  document.formIPFwAdd.l_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.l_ip.value,4,1,254) ) {
  alert('<% multilang("2222" "LANG_INVALID_LOCAL_IP_ADDRESS_RANGE_IN_4TH_DIGIT"); %>');
  document.formIPFwAdd.l_ip.focus();
  return false;
 }
 if (document.formIPFwAdd.r_ip.value=="") {
  alert('<% multilang("2223" "LANG_EMPTY_EXTERNAL_IP_ADDRESS"); %>');
  document.formIPFwAdd.r_ip.focus();
  return false;
 }
 if ( validateKey( document.formIPFwAdd.r_ip.value ) == 0 ) {
  alert('<% multilang("2224" "LANG_INVALID_EXTERNAL_IP_ADDRESS_VALUE_"); %>');
  document.formIPFwAdd.r_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.r_ip.value,1,0,255) ) {
  alert('<% multilang("2225" "LANG_INVALID_EXTERNAL_IP_ADDRESS_RANGE_IN_1ST_DIGIT"); %>');
  document.formIPFwAdd.r_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.r_ip.value,2,0,255) ) {
  alert('<% multilang("2226" "LANG_INVALID_EXTERNAL_IP_ADDRESS_RANGE_IN_2ND_DIGIT"); %>');
  document.formIPFwAdd.r_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.r_ip.value,3,0,255) ) {
  alert('<% multilang("2227" "LANG_INVALID_EXTERNAL_IP_ADDRESS_RANGE_IN_3RD_DIGIT"); %>');
  document.formIPFwAdd.r_ip.focus();
  return false;
 }
 if ( !checkDigitRange(document.formIPFwAdd.r_ip.value,4,1,254) ) {
  alert('<% multilang("2228" "LANG_INVALID_EXTERNAL_IP_ADDRESS_RANGE_IN_4TH_DIGIT"); %>');
  document.formIPFwAdd.r_ip.focus();
  return false;
 }
 obj.isclick = 1;
...
<div class="intro_main ">
 ...
 <p class="intro_content"> <% multilang("635" "LANG_ENTRIES_IN_THIS_TABLE_ALLOW_YOU_TO_AUTOMATICALLY_REDIRECT_TRAFFIC_TO_A_SPECIFIC_MACHINE_BEHIND_THE_NAT_FIREWALL_THESE_SETTINGS_ARE_ONLY_NECESSARY_IF_YOU_WISH_TO_HOST_SOME_SORT_OF_SERVER_LIKE_A_WEB_SERVER_OR_MAIL_SERVER_ON_THE_PRIVATE_LOCAL_NETWORK_BEHIND_YOUR_GATEWAY_S_NAT_FIREWALL"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th colspan="2">
       <input type="checkbox" name="enabled" value="ON" <% checkWrite("ipFwEn"); %>
         ONCLICK=updateState()>&nbsp;&nbsp;<% multilang("255" "LANG_ENABLE"); %> <% multilang("24" "LANG_NAT_IP_FORWARDING"); %>
   </th>
  </tr>
  <tr>
  ...
  <tr>
      <th><% multilang("636" "LANG_EXTERNAL"); %> <% multilang("89" "LANG_IP_ADDRESS"); %>: </th>
      <td><input type="text" name="r_ip" size="10" maxlength="15"></td>
  </tr>
 </table>
</div>
<div class="btn_ctl">
  <input class="link_bg" type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="addEntry" onClick="return addClick(this)">
...
<div class="column clearfix column_title">
 <div class="column_title_left"></div>
  <p><% multilang("637" "LANG_CURRENT_NAT_IP_FORWARDING_TABLE"); %></p>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("231" "LANG_DELETE_SELECTED"); %>" name="delSelEntry" onClick="return deleteClick(this)">&nbsp;&nbsp;
 <input class="link_bg" type="submit" value="<% multilang("232" "LANG_DELETE_ALL"); %>" name="delAllEntry" onClick="return deleteAllClick(this)">&nbsp;&nbsp;&nbsp;
