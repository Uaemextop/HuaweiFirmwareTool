<%SendWebHeadStr(); %>
<title>IPv6 IP/Port <% multilang("387" "LANG_FILTERING"); %></title>
<script>
  ...
  with ( document.forms[1] ) {
   if(dip6Start.value != ""){
    if (! isGlobalIpv6Address(dip6Start.value) ){
     alert('<% multilang("2234" "LANG_INVALID_DESTINATION_IPV6_START_ADDRESS"); %>');
     document.formFilterAdd.dip6Start.focus();
     return false;
    }
    if ( dip6PrefixLen.value != "" ) {
     if ( validateKey( document.formFilterAdd.dip6PrefixLen.value ) == 0 ) {
      alert('<% multilang("2235" "LANG_INVALID_DESTINATION_IPV6_PREFIX_LENGTH_IT_MUST_BE_0_9"); %>');
      document.formFilterAdd.dip6PrefixLen.focus();
      return false;
     }
     var prefixlen= getDigit(dip6PrefixLen.value, 1);
     if (prefixlen > 64 || prefixlen < 0) {
      alert('<% multilang("2236" "LANG_INVALID_DESTINATION_IPV6_PREFIX_LENGTH_IT_MUST_BE_0_64"); %>');
      document.formFilterAdd.dip6PrefixLen.focus();
      return false;
     }
    }
  ...
  if(type&1)
  {
   if(sIfId6Start.value != "")
   {
    if (!sIfId6Start.value.match(ifid_regex) ){
     alert('<% multilang("2239" "LANG_INVALID_SOURCE_IPV6_INTERFACE_ID_START_ADDRESS"); %>');
...
function addClick(obj)
{
 ...
 if(Src_Dst_All_Empty == 1)
 {
  if(checkPortType == 0)
  {
   alert('<% multilang("2238" "LANG_INPUT_FILTER_RULE_IS_NOT_VALID"); %>');
   return false;
  }
  else
  {
...
<div class="intro_main ">
 <p class="intro_title">IPv6 IP/Port <% multilang("387" "LANG_FILTERING"); %></p>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width="30%"><% multilang("403" "LANG_DEFAULT_ACTION"); %>:&nbsp;&nbsp;</th>
   <td><input type="radio" name="outAct" value=0 <% checkWrite("v6_ipf_out_act0"); %>><% multilang("390" "LANG_DENY"); %>&nbsp;&nbsp;
       <input type="radio" name="outAct" value=1 <% checkWrite("v6_ipf_out_act1"); %>><% multilang("391" "LANG_ALLOW"); %>&nbsp;&nbsp;</td>
   <td><input class="inner_btn" type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="setDefaultAction" onClick="return on_submit(this)">&nbsp;&nbsp;
...
<table>
   <tr>
  <th width="30%"><% multilang("397" "LANG_SOURCE"); %> :</th>
  <td>
   <select name="saType" onChange="saTypeSelect()">
   <option select value=1><% multilang("89" "LANG_IP_ADDRESS"); %>+<% multilang("472" "LANG_PREFIX_LENGTH"); %></option>
   <option value=2>PD+<% multilang("490" "LANG_INTERFACE_ID"); %></option>
   </select>
  </td>
   </tr>
   <tr id="sifid">
   <th><% multilang("397" "LANG_SOURCE"); %> <% multilang("490" "LANG_INTERFACE_ID"); %>:</th>
   <td><input type="text" size="16" name="sIfId6Start" style="width:150px"> </td>
   </tr>
   <tr id="sip6">
   <th><% multilang("397" "LANG_SOURCE"); %> <% multilang("89" "LANG_IP_ADDRESS"); %>:</th>
   <td><input type="text" size="16" name="sip6Start" style="width:150px">
   </tr>
   <tr id="sip6prefix">
   <th><% multilang("397" "LANG_SOURCE"); %> <% multilang("472" "LANG_PREFIX_LENGTH"); %>:</th>
   <td><input type="text" size="16" name="sip6PrefixLen" style="width:150px"></td>
   </tr>
   <tr>
  <th><% multilang("398" "LANG_DESTINATION"); %> :</th>
  <td>
   ...
   <tr id="difid">
   <th><% multilang("398" "LANG_DESTINATION"); %> <% multilang("490" "LANG_INTERFACE_ID"); %>:</th>
   <td><input type="text" size="16" name="dIfId6Start" style="width:150px"> </td>
   </tr>
   <tr id="dip6">
   <th><% multilang("398" "LANG_DESTINATION"); %> <% multilang("89" "LANG_IP_ADDRESS"); %>:</th>
   <td><input type="text" size="16" name="dip6Start" style="width:150px">
   </tr>
   <tr id="dip6prefix">
   <th><% multilang("398" "LANG_DESTINATION"); %> <% multilang("472" "LANG_PREFIX_LENGTH"); %>:</th>
   <td><input type="text" size="16" name="dip6PrefixLen" style="width:150px"></td>
   </tr>
   <tr>
   <th><% multilang("397" "LANG_SOURCE"); %> <% multilang("220" "LANG_PORT"); %>:</th>
   <td><input type="text" size="6" name="sfromPort" style="width:150px"> - <input type="text" size="6" name="stoPort" style="width:150px"></td>
   </tr>
   <tr>
   <th><% multilang("398" "LANG_DESTINATION"); %> <% multilang("220" "LANG_PORT"); %>:</th>
