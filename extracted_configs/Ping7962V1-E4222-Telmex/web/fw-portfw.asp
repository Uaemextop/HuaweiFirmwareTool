...
function addClick(obj)
{
 ...
 for(var i=0;i<mutli_localFromPort.length;i++)
 {
  ...
  if (mutli_localIPAddr[i].value!="")
  {
   ...
   {
    alert('<% multilang("2249" "LANG_INVALID_LOCAL_IP_ADDRESS"); %>');
    mutli_localIPAddr[i].focus();
    return false;
   }
  }
  if ( mutli_remoteIPAddr[i].value!="" ) {
   if (!checkNetIP(mutli_remoteIPAddr[i], 0))
   {
    alert('<% multilang("2250" "LANG_INVALID_REMOTE_IP_ADDRESS"); %>');
...
<div class="intro_main ">
 ...
 <p class="intro_content"> <% multilang("404" "LANG_PAGE_DESC_NAT_FIREWALL_FILTER_TABLE"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   ...
   <td width="40%">
    <input type="radio" value="0" name="portFwcap" <% checkWrite("portFw-cap0"); %> onClick="updateState()"><% multilang("254" "LANG_DISABLE"); %>&nbsp;&nbsp;
    <input type="radio" value="1" name="portFwcap" <% checkWrite("portFw-cap1"); %> onClick="updateState()"><% multilang("255" "LANG_ENABLE"); %>&nbsp;&nbsp;
   </td>
   <td>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width="30%"><% multilang("255" "LANG_ENABLE"); %>
    <input type="checkbox" name="fw_enable" value="1" checked>
   </th>
   <th width="70%"><% multilang("412" "LANG_APPLICATION"); %>
...
<table style ="table-layout:fixed; overflow:hidden;">
  ...
  <th width="15%">
   <% multilang("413" "LANG_LOCAL_IP"); %>
  </th>
  <th width="10%">
   <% multilang("414" "LANG_LOCAL_PORT_FROM"); %>
  </th>
  <th width="10%">
   <% multilang("415" "LANG_LOCAL_PORT_TO"); %>
  </th>
  <th width="10%">
   <% multilang("96" "LANG_PROTOCOL"); %>
  </th>
   <th width="10%" <% checkWrite("rg_hidden_function"); %>>
   <% multilang("416" "LANG_REMOTE_IP"); %>
  </th>
   <th width="10%">
   <% multilang("417" "LANG_REMOTE_PORT_FROM"); %>
  </th>
   <th width="10%">
   <% multilang("418" "LANG_REMOTE_PORT_TO"); %>
  </th>
   <th>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("228" "LANG_ADD"); %>" name="addPortFw" onClick="return addClick(this)">
...
<div class="column">
 <div class="column_title">
  ...
  <p> <% multilang("410" "LANG_CURRENT_PORT_FORWARDING_TABLE"); %></p>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("231" "LANG_DELETE_SELECTED"); %>" name="deleteSelPortFw" onClick="return deleteClick(this)">
 <input class="link_bg" type="submit" value="<% multilang("232" "LANG_DELETE_ALL"); %>" name="deleteAllPortFw" onClick="return deleteAllClick(this)">
</div>
 <script>
