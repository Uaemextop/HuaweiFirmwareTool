<%SendWebHeadStr(); %>
<title>L2TP VPN <% multilang("245" "LANG_CONFIGURATION"); %></title>
<SCRIPT>
...
function addL2TPItf(obj)
{
 ...
 if (document.l2tp.server.value=="") {
  alert('<% multilang("2281" "LANG_PLEASE_ENTER_L2TP_SERVER_ADDRESS"); %>');
 ...
 if (document.l2tp.IpProtocolType.value == 2)
 {
  ...
  {
   alert('<% multilang("2302" "LANG_PLEASE_INPUT_IPV6_ADDRESS"); %>');
   document.l2tp.server.focus();
   return false;
  }
 }
 else if (document.l2tp.IpProtocolType.value == 1)
 {
  ...
  {
   alert('<% multilang("2303" "LANG_PLEASE_INPUT_IPV4_ADDRESS"); %>');
   document.l2tp.server.focus();
   return false;
  }
 }
 if (document.l2tp.tunnel_auth.checked)
 {
  ...
  {
   alert('<% multilang("2283" "LANG_PLEASE_ENTER_L2TP_TUNNEL_AUTHENTICATION_SECRET"); %>');
   document.l2tp.tunnel_secret.focus();
   return false;
  }
  if(!checkTextStr(document.l2tp.tunnel_secret.value))
  {
   alert('<% multilang("2284" "LANG_INVALID_VALUE_IN_TUNNEL_AUTHENTICATION_SECRET"); %>');
   document.l2tp.tunnel_secret.focus();
   return false;
  }
 }
 if (document.l2tp.auth.selectedIndex!=3)
 {
  ...
  {
   alert('<% multilang("2285" "LANG_PLEASE_ENTER_L2TP_CLIENT_USERNAME"); %>');
   document.l2tp.username.focus();
   return false;
  }
  if(!checkTextStr(document.l2tp.username.value))
  {
   alert('<% multilang("2286" "LANG_INVALID_VALUE_IN_USERNAME"); %>');
   document.l2tp.username.focus();
   return false;
  }
  if (document.l2tp.password.value=="") {
   alert('<% multilang("2287" "LANG_PLEASE_ENTER_L2TP_CLIENT_PASSWORD"); %>');
   document.l2tp.password.focus();
   return false;
  }
  if(!checkTextStr(document.l2tp.password.value))
  {
   alert('<% multilang("2288" "LANG_INVALID_VALUE_IN_PASSWORD"); %>');
   document.l2tp.password.focus();
   return false;
  }
 }
 if (document.l2tp.pppconntype.selectedIndex==1)
 {
  ...
  {
   alert('<% multilang("2289" "LANG_PLEASE_ENTER_L2TP_TUNNEL_IDLE_TIME"); %>');
 ...
 {
  alert('<% multilang("2291" "LANG_PLEASE_ENTER_L2TP_TUNNEL_MTU"); %>');
...
<div class="intro_main ">
 <p class="intro_title">L2TP VPN <% multilang("245" "LANG_CONFIGURATION"); %></p>
 <p class="intro_content"> <% multilang("711" "LANG_THIS_PAGE_IS_USED_TO_CONFIGURE_THE_PARAMETERS_FOR_L2TP_MODE_VPN"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th>L2TP VPN:</th>
   <td>
    <input type="radio" value="0" name="l2tpen" <% checkWrite("l2tpenable0"); %> onClick="onClickL2TPEnable()"><% multilang("254" "LANG_DISABLE"); %>&nbsp;&nbsp;
    <input type="radio" value="1" name="l2tpen" <% checkWrite("l2tpenable1"); %> onClick="onClickL2TPEnable()"><% multilang("255" "LANG_ENABLE"); %>
   </td>
  </tr>
 </table>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th>IP <% multilang("96" "LANG_PROTOCOL"); %>:</th>
  ...
  <tr>
   <th><% multilang("712" "LANG_TUNNEL_AUTHENTICATION"); %>:</th>
   <td><input type=checkbox name="tunnel_auth" value=1 onClick=tunnelAuthChange()></td>
  </tr>
  <tr>
   <th><% multilang("713" "LANG_TUNNEL_AUTHENTICATION_SECRET"); %>:</th>
   <td><input type="text" name="tunnel_secret" size="15" maxlength="35"></td>
  </tr>
  <tr>
   <th>PPP <% multilang("209" "LANG_AUTHENTICATION"); %>:</th>
  ...
  <tr>
   <th>PPP <% multilang("208" "LANG_ENCRYPTION"); %>:</th>
  ...
  <tr>
   <th><% multilang("836" "LANG_USER"); %><% multilang("700" "LANG_NAME"); %>:</th>
  ...
  <tr>
   <th>PPP <% multilang("291" "LANG_CONNECTION_TYPE"); %>:</th>
   <td><select name="pppconntype" onChange="connTypeChange()">
     <option value="0"><% multilang("714" "LANG_PERSISTENT"); %></option>
     <option value="1"><% multilang("715" "LANG_DIAL_ON_DEMAND"); %></option>
     <option value="2"><% multilang("471" "LANG_MANUAL"); %></option>
     <option value="3"><% multilang("353" "LANG_NONE"); %></option>
    </select>
   </td>
  </tr>
  <tr>
   <th><% multilang("350" "LANG_IDLE_TIME_MIN"); %>:</th>
...
<div class="btn_ctl">
      <input class="link_bg" type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="addL2TP" onClick="return addL2TPItf(this)">&nbsp;&nbsp;
</div>
<div class="column">
 <div class="column_title">
  <div class="column_title_left"></div>
   <p>L2TP <% multilang("337" "LANG_TABLE"); %></p>
  <div class="column_title_right"></div>
 </div>
 <div class="data_common">
  <table>
   <tr>
    <th align=center width="3%"><% multilang("233" "LANG_SELECT"); %></th>
    <th align=center width="5%"><% multilang("70" "LANG_INTERFACE"); %></th>
    <th align=center width="5%"><% multilang("91" "LANG_SERVER"); %></th>
    <th align=center width="5%"><% multilang("712" "LANG_TUNNEL_AUTHENTICATION"); %></th>
    <th align=center width="5%">PPP <% multilang("209" "LANG_AUTHENTICATION"); %></th>
    <th align=center width="5%">MTU</th>
    <th align=center width="5%"><% multilang("85" "LANG_DEFAULT_GATEWAY"); %></th>
    <th align=center width="8%"><% multilang("710" "LANG_ACTION"); %></th>
   </tr>
   <% l2tpList(); %>
  </table>
 </div>
...
<div class="btn_ctl">
 <input class="link_bg" ype="submit" value="<% multilang("231" "LANG_DELETE_SELECTED"); %>" name="delSel" onClick="return deleteClick(this)">&nbsp;&nbsp;
