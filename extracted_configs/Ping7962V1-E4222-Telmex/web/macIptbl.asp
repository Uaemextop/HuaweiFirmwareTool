<%SendWebHeadStr(); %>
<title><% multilang("368" "LANG_MAC_BASED_ASSIGNMENT"); %></title>
<SCRIPT>
var pool_ipprefix;
...
function addClick(obj)
{
 ...
 var macdigit = 0;
   if ( str.length != 17) {
  alert("<% multilang("2305" "LANG_INPUT_HOST_MAC_ADDRESS_IS_NOT_COMPLETE_IT_SHOULD_BE_17_DIGITS_IN_HEX"); %>");
  document.macBase.hostMac.focus();
  return false;
   }
 if (document.macBase.hostMac.value=="") {
  alert("<% multilang("2306" "LANG_ENTER_HOST_MAC_ADDRES"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 ...
 if ( validateKey2( document.macBase.hostMac.value ) == 0 ) {
  alert("<% multilang("2307" "LANG_INVALID_HOST_MAC_ADDRESS_IT_SHOULD_BE_IN_HEX_NUMBER_0_9_OR_A_F_OR_A_F"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 if ( !checkInputIP(document.macBase.hostIp.value ) ) {
  alert("<% multilang("2308" "LANG_INVALID_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE"); %>");
  document.macBase.hostIp.focus();
  return false;
 }
 if ( !checkDigitRangeforMac(document.macBase.hostMac.value,1,0,255) ) {
  alert("<% multilang("2309" "LANG_INVALID_SOURCE_RANGE_IN_1ST_HEX_NUMBER_OF_HOST_MAC_ADDRESS_IT_SHOULD_BE_0X00_0XFF"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 if ( !checkDigitRangeforMac(document.macBase.hostMac.value,2,0,255) ) {
  alert("<% multilang("2310" "LANG_INVALID_SOURCE_RANGE_IN_2ND_HEX_NUMBER_OF_HOST_MAC_ADDRESS_IT_SHOULD_BE_0X00_0XFF"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 if ( !checkDigitRangeforMac(document.macBase.hostMac.value,3,0,255) ) {
  alert("<% multilang("2311" "LANG_INVALID_SOURCE_RANGE_IN_3RD_HEX_NUMBER_OF_HOST_MAC_ADDRESS_IT_SHOULD_BE_0X00_0XFF"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 if ( !checkDigitRangeforMac(document.macBase.hostMac.value,4,0,254) ) {
  alert("<% multilang("2312" "LANG_INVALID_SOURCE_RANGE_IN_4TH_HEX_NUMBER_OF_HOST_MAC_ADDRESS_IT_SHOULD_BE_0X00_0XFF"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 if ( !checkDigitRangeforMac(document.macBase.hostMac.value,5,0,255) ) {
  alert("<% multilang("2313" "LANG_INVALID_SOURCE_RANGE_IN_5RD_HEX_NUMBER_OF_HOST_MAC_ADDRESS_IT_SHOULD_BE_0X00_0XFF"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 if ( !checkDigitRangeforMac(document.macBase.hostMac.value,6,0,255) ) {
  alert("<% multilang("2314" "LANG_INVALID_SOURCE_RANGE_IN_6TH_HEX_NUMBER_OF_HOST_MAC_ADDRESS_IT_SHOULD_BE_0X00_0XFF"); %>");
  document.macBase.hostMac.focus();
  return false;
 }
 obj.isclick = 1;
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("368" "LANG_MAC_BASED_ASSIGNMENT"); %></p>
 <p class="intro_content"> <% multilang("1158" "LANG_THIS_PAGE_IS_USED_TO_CONFIGURE_THE_STATIC_IP_BASE_ON_MAC_ADDRESS_YOU_CAN_ASSIGN_DELETE_THE_STATIC_IP_THE_HOST_MAC_ADDRESS_PLEASE_INPUT_A_STRING_WITH_HEX_NUMBER_SUCH_AS_00_D0_59_C6_12_43"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th align=left><% multilang("93" "LANG_MAC_MAP_ENABLE"); %>:</th>
   <td><input type="checkbox" id="enable_chk" name="enable" size="20" maxlength="17">&nbsp;&nbsp;</td>
  </tr>
  <tr>
   <th align=left><% multilang("92" "LANG_MAC_ADDRESS"); %> (xx-xx-xx-xx-xx-xx): </th>
   <td><input type="text" name="hostMac" size="20" maxlength="17">&nbsp;&nbsp;</td>
  </tr>
  <tr>
   <th align=left><% multilang("828" "LANG_ASSIGNED_IP_ADDRESS"); %> (xxx.xxx.xxx.xxx): </th>
   <td><input type="text" name="hostIp" size="20" maxlength="15">&nbsp;&nbsp;</td>
  </tr>
 </table>
</div>
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("820" "LANG_ASSIGN"); %> IP" name="addIP" onClick="return addClick(this)">&nbsp;&nbsp;
 <input class="link_bg" type="submit" value="<% multilang("829" "LANG_DELETE_ASSIGNED_IP"); %>" name="delIP" onClick="return on_submit(this)">&nbsp;&nbsp;
 <input class="link_bg" type="submit" value="<% multilang("830" "LANG_MODIFY_ASSIGNED_IP"); %>" name="modIP" onClick="return addClick(this)">&nbsp;&nbsp;
...
<div class="column">
 <div class="column_title">
  <div class="column_title_left"></div>
   <p><% multilang("368" "LANG_MAC_BASED_ASSIGNMENT"); %><% multilang("1159" "LANG_TABLE_2"); %></p>
