<%SendWebHeadStr(); %>
<title><% multilang("491" "LANG_OTHER_ADVANCED"); %> <% multilang("245" "LANG_CONFIGURATION"); %></title>
<SCRIPT>
function saveChanges(obj)
{
 if ( checkDigit(document.others.ltime.value) == 0) {
  alert("<% multilang("2366" "LANG_INVALID_LEASE_TIME"); %>");
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("491" "LANG_OTHER_ADVANCED"); %> <% multilang("245" "LANG_CONFIGURATION"); %></p>
 <p class="intro_content"> <% multilang("492" "LANG_HERE_YOU_CAN_SET_SOME_OTHER_ADVANCED_SETTINGS"); %></p>
</div>
<form action=/boaform/formOthers method=POST name="others">
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th><% multilang("493" "LANG_IP_PASSTHROUGH"); %>:</th>
   <td>
    <select name="ippt" onChange=ipptSelected()>
     <option value=65535><% multilang("353" "LANG_NONE"); %></option>
     <% if_wan_list("p2p"); %>
    </select>
   </td>
  </tr>
  <tr>
   <th><% multilang("494" "LANG_LEASE_TIME"); %>:</th>
   <td>
    <input type="text" name="ltime" size=10 maxlength=9 value="<% getInfo("ippt-lease"); %>"> <% multilang("365" "LANG_SECONDS"); %>
   </td>
  </tr>
  <tr>
   <th><% multilang("495" "LANG_ALLOW_LAN_ACCESS"); %>:</th>
   <td>
