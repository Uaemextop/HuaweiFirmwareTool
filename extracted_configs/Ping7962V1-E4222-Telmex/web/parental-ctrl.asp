<%SendWebHeadStr(); %>
<title><% multilang("22" "LANG_PARENTAL_CONTROL"); %></title>
<script>
...
function addClick(obj)
{
  ...
  {
   alert("<% multilang("2367" "LANG_INPUT_USERNAME"); %>");
 ...
 return false;
  ...
  if (document.formParentCtrlAdd.specPC[0].checked)
  {
    ...
    return false;
     ...
     {
     alert("<% multilang("2368" "LANG_LOCAL_START_IP_DOMAIN_IS_DIFFERENT_FROM_END_IP"); %>");
     document.formParentCtrlAdd.ipstart.focus();
     return false;
     }
     if ( le <= ls )
     {
   alert("<% multilang("2369" "LANG_INVALID_IP_RANGE"); %>");
   document.formParentCtrlAdd.ipstart.focus();
   return false;
     }
  }
  ...
  {
   alert("<% multilang("2245" "LANG_INPUT_MAC_ADDRESS"); %>");
 document.formParentCtrlAdd.mac.focus();
 return false;
  }
  var str = document.formParentCtrlAdd.mac.value;
  if ( str.length < 12) {
 alert("<% multilang("2246" "LANG_INPUT_MAC_ADDRESS_IS_NOT_COMPLETE_IT_SHOULD_BE_12_DIGITS_IN_HEX"); %>");
 document.formParentCtrlAdd.mac.focus();
 return false;
  }
  for (var i=0; i<str.length; i++) {
   ...
   (str.charAt(i) >= 'A' && str.charAt(i) <= 'F') )
   continue;
 alert("<% multilang("2370" "LANG_INVALID_MAC_ADDRESS_IT_SHOULD_BE_IN_HEX_NUMBER_0_9_OR_A_F"); %>");
 document.formParentCtrlAdd.mac.focus();
 return false;
  }
  }
  if ((document.formParentCtrlAdd.starthr.value=="" )
   ...
   alert("<% multilang("2371" "LANG_INVALID_VALUE_OF_TIME_VALUE_PLEASE_INPUT_CORRENT_TIME_FORMAT_00_00_23_59"); %>");
 ...
 return false;
  ...
  if ( num1 > num3 ) {
   alert("<% multilang("2372" "LANG_END_TIME_SHOULD_BE_LARGER"); %>");
   document.formParentCtrlAdd.starthr.focus();
 return false;
  }
 ...
 return false;
  ...
  if ((num2 > 59)||(num4 > 59) ) {
   alert("<% multilang("2373" "LANG_INVALID_VALUE_OF_TIME_VALUE_PLEASE_INPUT_CORRENT_TIME_FORMAT_EX_12_50"); %>");
   document.formParentCtrlAdd.starthr.focus();
 return false;
  }
  obj.isclick = 1;
...
<div class="intro_main ">
    <p class="intro_title"><% multilang("22" "LANG_PARENTAL_CONTROL"); %></p>
    <p class="intro_content"><% multilang("616" "LANG_ENTRIES_IN_THIS_TABLE_ARE_USED_TO_RESTRICT_ACCESS_TO_INTERNET_FROM_YOUR_LOCAL_PCS_DEVICES_BY_MAC_ADDRESS_AND_TIME_INTERVAL_USE_OF_SUCH_FILTERS_CAN_BE_HELPFUL_FOR_PARENTS_TO_CONTROL_CHILDREN_S_USAGE_OF_INTERNET"); %></p>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
  <th><% multilang("22" "LANG_PARENTAL_CONTROL"); %>:</th>
  <td>
   <input type="radio" value="0" name="parental_ctrl_on" <% checkWrite("parental-ctrl-on-0"); %> ><% multilang("254" "LANG_DISABLE"); %>&nbsp;&nbsp;
   <input type="radio" value="1" name="parental_ctrl_on" <% checkWrite("parental-ctrl-on-1"); %> ><% multilang("255" "LANG_ENABLE"); %>&nbsp;&nbsp;
  </td>
  <td>
   <input type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="parentalCtrlSet" onClick="return on_submit(this)"></td>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width=20%><% multilang("836" "LANG_USER"); %><% multilang("700" "LANG_NAME"); %>:</th>
   <td><input type="text" name="usrname" size="32" maxlength="31"></td>
  </tr>
  <tr>
   <th width=20%><% multilang("2" "LANG_SPECIFIED_PC"); %>:</th>
   <td>
    <input type="radio" value="0" name="specPC" checked onClick="updateState()"><% multilang("89" "LANG_IP_ADDRESS"); %>&nbsp;&nbsp;
    <input type="radio" value="1" name="specPC" onClick="updateState()"><% multilang("92" "LANG_MAC_ADDRESS"); %>&nbsp;&nbsp;
   </td>
  </tr>
  <tr>
   <th width=20%><% multilang("89" "LANG_IP_ADDRESS"); %>:</th>
   <td>
 ...
 <table>
  <tr>
   <th width=20%></th>
   <td align='center'><% multilang("617" "LANG_SUN"); %></font></td>
   <td align='center'><% multilang("618" "LANG_MON"); %></font></td>
   <td align='center'><% multilang("619" "LANG_TUE"); %></font></td>
   <td align='center'><% multilang("620" "LANG_WED"); %></font></td>
   <td align='center'><% multilang("621" "LANG_THU"); %></font></td>
   <td align='center'><% multilang("622" "LANG_FRI"); %></font></td>
   <td align='center'><% multilang("623" "LANG_SAT"); %></font></td>
  </tr>
  <tr>
   <th width=20%><% multilang("625" "LANG_CONTROLLED_DAYS"); %>:</th>
 ...
 <table>
  <tr>
   <th width=20%><% multilang("626" "LANG_START_BLOCKING_TIME"); %>:</th>
   <td>
  ...
  <tr>
   <th width=20%><% multilang("627" "LANG_END_BLOCKING_TIME"); %>:</th>
   <td>
...
<div class="btn_ctl">
    <input class="link_bg" type="submit" value="<% multilang("228" "LANG_ADD"); %>" name="addfilterMac" onClick="return addClick(this)">&nbsp;&nbsp;
 <input type="hidden" value="/parental-ctrl.asp" name="submit-url">
 <input type="hidden" name="postSecurityFlag" value="">
</div>
<script>
...
<div class="column">
 <div class="column_title">
  <div class="column_title_left"></div>
   <p><% multilang("628" "LANG_CURRENT_PARENT_CONTROL_TABLE"); %></p>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("231" "LANG_DELETE_SELECTED"); %>" name="deleteSelFilterMac" onClick="return deleteClick(this)">&nbsp;&nbsp;
 <input class="link_bg" type="submit" value="<% multilang("232" "LANG_DELETE_ALL"); %>" name="deleteAllFilterMac" onClick="return deleteAllClick(this)">&nbsp;&nbsp;&nbsp;
 <input type="hidden" value="/parental-ctrl.asp" name="submit-url">
 <input type="hidden" name="postSecurityFlag" value="">
</div>
<script>
