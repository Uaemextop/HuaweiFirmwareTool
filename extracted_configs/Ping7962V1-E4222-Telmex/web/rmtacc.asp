<%SendWebHeadStr(); %>
<title><% multilang("43" "LANG_REMOTE_ACCESS"); %><% multilang("245" "LANG_CONFIGURATION"); %></title>
<script>
function addClick(obj)
{
 ...
 if (dTelnet == dFtp || dTelnet == dWeb) {
  alert("<% multilang("2411" "LANG_DUPLICATED_PORT_NUMBER"); %>");
 ...
 if (document.acc.w_telnet.checked) {
  if (document.acc.w_telnet_port.value=="") {
   alert("<% multilang("2412" "LANG_PORT_RANGE_CANNOT_BE_EMPTY_YOU_SHOULD_SET_A_VALUE_BETWEEN_1_65535"); %>");
   document.acc.w_telnet_port.focus();
   return false;
  }
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("43" "LANG_REMOTE_ACCESS"); %> <% multilang("245" "LANG_CONFIGURATION"); %></p>
 <p class="intro_content"> <% multilang("455" "LANG_THIS_PAGE_IS_USED_TO_ENABLE_DISABLE_MANAGEMENT_SERVICES_FOR_THE_LAN_AND_WAN"); %></p>
</div>
...
<div class="data_vertical data_common_notitle">
  ...
  <table>
  <tr>
   <th align=left><% multilang("386" "LANG_SERVICE"); %><% multilang("700" "LANG_NAME"); %></th>
   <th align=center><% multilang("6" "LANG_LAN"); %></th>
   <th align=center><% multilang("11" "LANG_WAN"); %></th>
   <th align=center><% multilang("456" "LANG_WAN_PORT"); %></th>
  </tr>
  <% rmtaccItem(); %>
  </table>
 </div>
...
<div class="btn_ctl">
 <input class="link_bg" type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="set" onClick="return addClick(this)">
