<% SendWebHeadStr();%>
<title><% multilang("69" "LANG_TIME_ZONE"); %><% multilang("245" "LANG_CONFIGURATION"); %></title>
<script>
...
function checkNumber(field){
    ...
    for (var i=0; i<str.length; i++) {
     if ( (str.charAt(i) >= '0' && str.charAt(i) <= '9'))
                        continue;
 field.value = field.defaultValue;
        alert("<% multilang("2461" "LANG_IT_SHOULD_BE_IN_NUMBER_0_9"); %>");
        return false;
    }
 return true;
}
...
function saveChanges(form,obj){
 ...
 if(form.month.value == '0'){
  form.month.value = form.month.defaultValue;
         alert("<% multilang("2462" "LANG_INVALID_MONTH_NUMBER_IT_SHOULD_BE_IN_NUMBER_1_9"); %>");
  return false;
 }
 if (!checkMonth(form.month.value)) {
  alert("<% multilang("2463" "LANG_INVALID_MONTH_SETTING"); %>");
  form.month.focus();
  return false;
 }
 if (!checkDay(form.day.value, form.month.value)) {
  alert("<% multilang("2464" "LANG_INVALID_DAY_SETTING"); %>");
  form.day.focus();
  return false;
 }
 if (!checkHour(form.hour.value)) {
  alert("<% multilang("2465" "LANG_INVALID_HOUR_SETTING"); %>");
  form.hour.focus();
  return false;
 }
 if (!checkTime(form.minute.value) || !checkTime(form.second.value)) {
  alert("<% multilang("2466" "LANG_INVALID_TIME_SETTING"); %>");
  return false;
 }
 if (form.enabled.checked && form.ntpServerId[1].checked && form.ntpServerHost2.value != form.ntpServerHost2.defaultValue) {
  if (form.ntpServerHost2.value == "" || !checkString(form.ntpServerHost2.value)) {
   alert("<% multilang("2467" "LANG_INVALID_SERVER_STRING"); %>");
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("69" "LANG_TIME_ZONE"); %> <% multilang("245" "LANG_CONFIGURATION"); %></p>
 <p class="intro_content"> <% multilang("569" "LANG_YOU_CAN_MAINTAIN_THE_SYSTEM_TIME_BY_SYNCHRONIZING_WITH_A_PUBLIC_TIME_SERVER_OVER_THE_INTERNET"); %></p>
</div>
<form action=/boaform/formNtp method=POST name="time">
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width ="25%"> <% multilang("570" "LANG_CURRENT_TIME"); %> : </th>
   <td width ="75%">
                <% multilang("577" "LANG_YEAR"); %><input type="text" name="year" value="<% getInfo("year"); %>" size="4" maxlength="4">
                <% multilang("578" "LANG_MONTH"); %><input type="text" name="month" value="<% getInfo("month"); %>" size="2" maxlength="2">
                <% multilang("579" "LANG_DAY"); %><input type="text" name="day" value="<% getInfo("day"); %>" size="2" maxlength="2">
                <% multilang("580" "LANG_HOUR"); %><input type="text" name="hour" value="<% getInfo("hour"); %>" size="2" maxlength="2">
                <% multilang("581" "LANG_MIN"); %><input type="text" name="minute" value="<% getInfo("minute"); %>" size="2" maxlength="2">
                <% multilang("582" "LANG_SEC"); %><input type="text" name="second" value="<% getInfo("second"); %>" size="2" maxlength="2">
         </td>
  </tr>
  <tr><th width ="25%"><% multilang("571" "LANG_TIME_ZONE_SELECT"); %> : </th>
      <td width="75%">
  ...
  <tr><th><% multilang("572" "LANG_ENABLE_DAYLIGHT_SAVING_TIME"); %></th>
   <td><input type="checkbox" name="dst_enabled" value="ON">
      </td>
  </tr>
  <tr><th><% multilang("573" "LANG_ENABLE_SNTP_CLIENT_UPDATE"); %></th>
   <td>
    <input type="checkbox" name="enabled" value="ON" ONCLICK=updateState(document.time)>
      </td>
  </tr>
  <tr>
   <th width="25%"><% multilang("432" "LANG_WAN_INTERFACE"); %>:</th>
   <td width="75%">
    <select name="ext_if" <% checkWrite("sntp0d"); %>>
     <option value=65535><% multilang("409" "LANG_ANY"); %></option>
     <% if_wan_list("rt"); %>
    </select>
   </td>
  </tr>
  <tr>
   ...
   <td width ="75%">
    ...
    <select name="ntpServerHost1">
     <option value="192.5.41.41"><% multilang("583" "LANG_192_5_41_41_NORTH_AMERICA"); %></option>
     <option value="192.5.41.209"><% multilang("584" "LANG_192_5_41_209_NORTH_AMERICA"); %></option>
     <option value="130.149.17.8"><% multilang("585" "LANG_130_149_17_8_EUROPE"); %></option>
     <option value="203.117.180.36"><% multilang("586" "LANG_203_117_180_36_ASIA_PACIFIC"); %></option>
    </select>
    &nbsp;&nbsp;&nbsp;&nbsp;
    <input type="radio" value="1" name="ntpServerId"></input>
    <!--ping_zhang:20081217 START:patch from telefonica branch to support WT-107-->
    <input type="text" name="ntpServerHost2" size="15" maxlength="30" value=<% getInfo("ntpServerHost2"); %>> (<% multilang("574" "LANG_MANUAL_SETTING"); %>)
   </td>
  </tr>
 </table>
</div>
<div class="adsl clearfix">
 ...
 <input class="link_bg" type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="save" onClick="return saveChanges(document.time,this)">
 <input class="link_bg" type="button" value="<% multilang("441" "LANG_REFRESH"); %>" name="refresh" onClick="javascript: window.location.reload()">
