<% SendWebHeadStr();%>
<title><% multilang("170" "LANG_WLAN_ADVANCED_SETTINGS"); %></title>
<SCRIPT>
...
function validateNum(str)
{
  ...
  alert("<% multilang("2505" "LANG_INVALID_VALUE_IT_SHOULD_BE_IN_DECIMAL_NUMBER_0_9"); %>");
  return false;
   }
  }
  return true;
...
function saveChanges(obj)
{
 ...
 return false;
  ...
  if (document.advanceSetup.fragThreshold.value == "" || num < 256 || num > 2346) {
   alert("<% multilang("2506" "LANG_INVALID_VALUE_OF_FRAGMENT_THRESHOLD_INPUT_VALUE_SHOULD_BE_BETWEEN_256_2346_IN_DECIMAL"); %>");
   document.advanceSetup.fragThreshold.focus();
 return false;
  }
 ...
 return false;
  ...
  if (document.advanceSetup.rtsThreshold.value=="" || num > 2347) {
   alert("<% multilang("2507" "LANG_INVALID_VALUE_OF_RTS_THRESHOLD_INPUT_VALUE_SHOULD_BE_BETWEEN_0_2347_IN_DECIMAL"); %>");
   document.advanceSetup.rtsThreshold.focus();
 return false;
  }
 ...
 return false;
  ...
  if (document.advanceSetup.beaconInterval.value=="" || num < 20 || num > 1024) {
   alert("<% multilang("2508" "LANG_INVALID_VALUE_OF_BEACON_INTERVAL_INPUT_VALUE_SHOULD_BE_BETWEEN_20_1024_IN_DECIMAL"); %>");
   document.advanceSetup.beaconInterval.focus();
 return false;
  }
  num = parseInt(document.advanceSetup.dtimPeriod.value);
  if (document.advanceSetup.dtimPeriod.value=="" || num < 1 || num > 255) {
 alert("<% multilang("2509" "LANG_INVALID_VALUE_OF_DTIM_PRERIOD_INPUT_VALUE_SHOULD_BE_BETWEEN_1_255_IN_DECIMAL"); %>");
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("170" "LANG_WLAN_ADVANCED_SETTINGS"); %></p>
 <p class="intro_content"><% multilang("171" "LANG_PAGE_DESC_WLAN_ADVANCE_SETTING"); %></p>
</div>
 ...
 <table>
  <input type=hidden name="wlanDisabled" value=<% wlanStatus(); %>>
     <tr <% checkWrite("wlan_qtn_hidden_function"); %>>
       <th width="30%"><% multilang("172" "LANG_FRAGMENT_THRESHOLD"); %>:</th>
       <td width="70%"><input type="text" name="fragThreshold" size="10" maxlength="4" value=<% getInfo("fragThreshold"); %>>(256-2346)</td>
     </tr>
     <tr>
       <th width="30%"><% multilang("173" "LANG_RTS_THRESHOLD"); %>:</th>
       <td width="70%"><input type="text" name="rtsThreshold" size="10" maxlength="4" value=<% getInfo("rtsThreshold"); %>>(0-2347)</td>
     </tr>
     <tr>
       <th width="30%"><% multilang("174" "LANG_BEACON_INTERVAL"); %>:</th>
       <td width="70%"><input type="text" name="beaconInterval" size="10" maxlength="4" value=<% getInfo("beaconInterval"); %>> (20-1024 <% multilang("979" "LANG_MS"); %>)</td>
     </tr>
     <tr>
       <th width="30%"><% multilang("179" "LANG_DTIM_PERIOD"); %>:</th>
       <td width="70%"><input type="text" name="dtimPeriod" size="10" maxlength="4" value=<% getInfo("dtimPeriod"); %>> (1-255)</td>
     </tr>
     <tr>
       <th width="30%"><% multilang("175" "LANG_DATA_RATE"); %>:</th>
       <td width="70%"><select size="1" name="txRate">
    <SCRIPT>
     <% checkWrite("wl_txRate"); %>
     if(!is_wlan_qtn){
      rate_mask = [31,1,1,1,1,2,2,2,2,2,2,2,2,4,4,4,4,4,4,4,4,8,8,8,8,8,8,8,8,16,16,16,16,16,16,16,16,32,32,32,32,32,32,32,32];
      rate_name =["<% multilang("176" "LANG_AUTO"); %>","1M","2M","5.5M","11M","6M","9M","12M","18M","24M","36M","48M","54M", "MCS0", "MCS1",
      ...
      if(band & 64){
       ...
       for (i=0; i<=vht_num; i++) {
         ...
         if (txrate == rate)
          defidx = idx;
         if(chanwid!=0 || (i!=9 && i!=19 && i!=29 && i!=39))
         {
      ...
      idx=0, i=0;
      document.write('<option value="' + (i) + '">' +'<% multilang("176" "LANG_AUTO"); %>'+ '\n');
      idx++;
      defidx=0;
      if ((band & 8) || (band&4)) {
       for (i=1; i<78; i++) {
   ...
   <!-- for WiFi test, start --
     <tr>
       <td width="30%"><font size=2><b><% multilang("177" "LANG_TX_OPERATION_RATE"); %>:</b></td>
  ...
  <input type="checkbox" name="operRate48M" value="48M">28M&nbsp;&nbsp;
     ...
     <tr>
       <td width="30%"><font size=2><b><% multilang("178" "LANG_TX_BASIC_RATE"); %>:</b></td>
       <td width="70%"><font size=2>
  ...
  <input type="checkbox" name="basicRate5M" value="5M">5.5M&nbsp;&nbsp;
   ...
   <input type="checkbox" name="basicRate48M" value="48M">28M&nbsp;&nbsp;
      ...
      <tr>
        <td width="30%"><font size=2><b><% multilang("179" "LANG_DTIM_PERIOD"); %>:</b></td>
