<% SendWebHeadStr();%>
<title><% multilang("139" "LANG_WLAN_BASIC_SETTINGS"); %></title>
<style>
.on {display:on}
.off {display:none}
</style>
...
function saveChanges(obj)
{
 ...
 if(byteLength(document.wlanSetup.ssid.value)>32){
  alert("<% multilang("2511" "LANG_SSID_LENGTH_SHOULD_BE_MAXIMUM_32_OCTETS"); %>");
  document.wlanSetup.ssid.focus();
  return false;
 }
 if(selectText == '2')
...
function disableWLAN()
{
 ...
 if(isCMCCSupport==1)
   disableTextField(document.wlanSetup.ssidpri);
...
function enableWLAN()
{
 ...
 if(isCMCCSupport==1)
   enableTextField(document.wlanSetup.ssidpri);
...
function showChannel2G(bound_40, band)
{
 ...
 document.wlanSetup.chan.length=0;
 idx=0;
 document.wlanSetup.chan.options[idx] = new Option("<% multilang("176" "LANG_AUTO"); %>", 0, false, false);
 if (0 == wlan_channel) {
  document.wlanSetup.chan.selectedIndex = 0;
 }
 idx++;
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("139" "LANG_WLAN_BASIC_SETTINGS"); %></p>
 <p class="intro_content"><% multilang("140" "LANG_PAGE_DESC_WLAN_BASIC_SETTING"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th colspan=2>
    ...
    ONCLICK=updateState(document.wlanSetup)>&nbsp;&nbsp;<% multilang("141" "LANG_DISABLE_WLAN_INTERFACE"); %>
   </th>
  </tr>
  <tr>
   <th width="26%"><% multilang("142" "LANG_BAND"); %>:</th>
  ...
  <tr>
   <th width="26%"><% multilang("143" "LANG_MODE"); %>:</th>
   <td width="74%"><select size="1" name="mode" onChange="updateMode()">
    <% checkWrite("wlmode"); %>
    </select>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <input type="button" value="<% multilang("162" "LANG_MULTIPLE_AP"); %>" name="multipleAP" onClick="showMultipleAP(document.wlanSetup, '/admin/wlmultipleap.asp')" class="inner_btn">
   </td>
  </tr>
  <tr>
 ...
 <div id="optionfor11n" style="display:none">
  <table>
   <tr>
    <th width="26%"><% multilang("145" "LANG_CHANNEL_WIDTH"); %>:</th>
   ...
   <tr <% checkWrite("wlan_qtn_hidden_function"); %> >
    <th width="26%"><% multilang("153" "LANG_CONTROL_SIDEBAND"); %>:</th>
 ...
 <table>
  <tr>
   <th width="26%"><% multilang("154" "LANG_CHANNEL_NUMBER"); %>:</th>
  ...
  <tr <% checkWrite("wlan_qtn_hidden_function"); %>>
   <th width="26%"><% multilang("160" "LANG_RADIO_POWER"); %>:</th>
  ...
  <script>
         if(isCMCCSupport==1){
    document.write('<tr>');
    document.write('<th width="26%"><% multilang("3015" "LANG_SSID_PRIORITY"); %>:</th>');
    document.write('<td width="74%"><select size="1" name="ssidpri">');
    document.write('<option value=\"0\">0</option>\n');
          document.write('<option value=\"1\">1</option>\n');
    document.write('<option value=\"2\">2</option>\n');
    document.write('<option value=\"3\">3</option>\n');
    document.write('<option value=\"4\">4</option>\n');
    priority = <% checkWrite("ssid_pri"); %>;
    def_index = priToIndex(priority);
    document.wlanSetup.ssidpri.selectedIndex = def_index;
  ...
  <% extra_wlbasic(); %>
  <tr id="wl_limit_stanum">
   <th width="26%"><% multilang("156" "LANG_STA_NUMBER"); %>:</th>
   <td>
  ...
  <tr>
   <th width="26%"><% multilang("157" "LANG_ASSOCIATED_CLIENTS"); %>:</th>
   <td width="74%"><input type="button"
   value="<% multilang("158" "LANG_SHOW_ACTIVE_WLAN_CLIENTS"); %>" name="showMac"
  ...
  <tr id="repeater_check" style="display:none">
   <th colspan=2>
    ...
    ONCLICK="updateRepeaterState(document.wlanSetup)">&nbsp;&nbsp; Enable Universal Repeater Mode (Acting as AP and client simultaneouly)
   </th>
  </tr>
  ...
  <tr id="wifi_regdomain_demo" style="display:none">
   <th width="26%"><% multilang("161" "LANG_REGDOMAIN"); %>:</th>
...
<div class="btn_ctl">
 ...
 <input type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>"
