...
var rate_mask = [31,1,1,1,1,2,2,2,2,2,2,2,2,4,4,4,4,4,4,4,4,8,8,8,8,8,8,8,8,16,16,16,16,16,16,16,16,32,32,32,32,32,32,32,32];
...
function DisplayTxRate(v_index, band, auto, txrate, rf_num, channel_width)
{
 ...
 if(band & 64){
  ...
  for (i=0; i<=vht_num; i++) {
    ...
    if (txrate == rate)
     defidx = idx;
    if(channel_width!=0 || (i!=9 && i!=19 && i!=29 && i!=39))
    {
...
<!--
 <tr>
  <td><b><% multilang("253" "LANG_BLOCKING_BETWEEN_VAP"); %>:</b></td>
  <td><input type="radio" name="mbssid_block" value="disable" ><% multilang("254" "LANG_DISABLE"); %>&nbsp;&nbsp;
  <input type="radio" name="mbssid_block" value="enable" ><% multilang("255" "LANG_ENABLE"); %></td>
 </tr>
-->
 <div class="table_data">
  <table>
   <tr style="background:#e9e9e9;">
      ...
      <th>WMM</th>
      <th><% multilang("193" "LANG_RELAY_BLOCKING"); %></th>
      <th id="wl_limit_stanum_div" style="display:none"><% multilang("156" "LANG_STA_NUMBER"); %></th>
      <th>Active Client List</th>
      <th id="wl_txbf_div" style="display:none"><% multilang("199" "LANG_TXBF"); %></th>
      <th id="wl_txbfmu_div" style="display:none"><% multilang("200" "LANG_TXBF_MU"); %></th>
      <th id="wl_mc2u_div" style="display:none"><% multilang("201" "LANG_MC2U"); %></th>
   </tr>
   <tr>
    <td>AP1</td>
    <td>
