<% SendWebHeadStr();%>
<title><% multilang("256" "LANG_WLAN_STATUS"); %></title>
<script>
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("256" "LANG_WLAN_STATUS"); %></p>
 <p class="intro_content"><% multilang("260" "LANG_THIS_PAGE_SHOWS_THE_WLAN_CURRENT_STATUS"); %></p>
</div>
<script>
 var wlan_num = 1;
 var isNewMeshUI = 1;
 ...
 for(i=0; i < wlan_num; i++){
  document.write(' <div class="column"> <div class="column_title"> <div class="column_title_left"></div> <p><% multilang("8" "LANG_WLAN"); %> <% multilang("245" "LANG_CONFIGURATION"); %></p> <div class="column_title_right"></div> </div> <div class="data_common"> <table>');
  document.write(' <tr> <th width=40%><% multilang("143" "LANG_MODE"); %></th> <td width=60%>' );
  if(wlanMode[i] == 0)
   document.write("AP");
  if(wlanMode[i] == 1){
   if (networkType[0] == 0)
  ...
  document.write('</td></tr> <tr> <th width=40%><% multilang("142" "LANG_BAND"); %></th> <td width=60%>');
 if (band[i] == 1)
 ...
 document.write('</td></tr> <tr> <th width=40%><% multilang("144" "LANG_SSID"); %></th> <td width=60% class="show_space">');
 if (wlanMode[i] != 2) {
  document.write(ssid_drv[i]);
 }
 document.write('</td> </tr> <tr> <th width=40%><% multilang("154" "LANG_CHANNEL_NUMBER"); %></th> <td width=60%>'+channel_drv[i] +'</td> </tr> <tr> <th width=40%><% multilang("208" "LANG_ENCRYPTION"); %></th> <td width=60%>');
 ...
 document.write('</td> </tr> <tr> <th width=40%><% multilang("1219" "LANG_BSSID"); %></th> <td width=60%>'+bssid_drv[i]+'</td> </tr>');
 if (wlanMode[i]!=2) {
  document.write('<tr >\n');
  if (wlanMode[i]==0 || wlanMode[i]==3 || wlanMode[i]==4) {
   document.write("<th width=40%%><% multilang("157" "LANG_ASSOCIATED_CLIENTS"); %></th>\n");
   document.write("<td width=60%%>"+clientnum[i]+"</td></tr>");
  }
  else {
   document.write("<th width=40%%><% multilang("261" "LANG_STATE"); %></th>\n");
 ...
 if (!wlanDisabled[i] && (wlanMode[i]==0 || wlanMode[i]==3 )) {
  for (idx=0; idx<mssid_num; idx++) {
   if (!mssid_disable[idx]) {
    document.write(' <div class="column"> <div class="column_title"> <div class="column_title_left"></div> <p><% multilang("262" "LANG_VIRTUAL_AP"); %>'+(idx+1)+' <% multilang("245" "LANG_CONFIGURATION"); %></p> <div class="column_title_right"></div> </div> <div class="data_common"> <table>');
    if(!is_wlan_qtn){
     document.write('<tr> <th width=40%><% multilang("142" "LANG_BAND"); %></th> <td width=60%>');
     if (mssid_band[idx] == 1)
         document.write( "2.4 GHz (B)");
        if (mssid_band[idx] == 2)
         document.write( "2.4 GHz (G)");
    ...
    document.write('</tr> <tr> <th><% multilang("208" "LANG_ENCRYPTION"); %></th> <td>'+mssid_wep[idx]+'</td>');
    document.write('</tr> <tr> <th>BSSID</th> <td>'+mssid_bssid_drv[idx]+'</td>');
    document.write('</tr> <tr> <th><% multilang("157" "LANG_ASSOCIATED_CLIENTS"); %></th> <td>'+mssid_clientnum[idx]+'</td></tr>');
    document.write('</table></div></div>');
   }
  }
 }
    if (rp_enabled[i])
    {
     document.write(' <div class="column"> <div class="column_title"> <div class="column_title_left"></div> <p><% multilang("8" "LANG_WLAN"); %> <% multilang("263" "LANG_REPEATER_INTERFACE"); %> <% multilang("245" "LANG_CONFIGURATION"); %></p> <div class="column_title_right"></div> </div> <div class="data_common"> <table>');
     document.write(' <tr> <th width=40%><% multilang("143" "LANG_MODE"); %></th> <td width=60%>' );
     if(rp_mode[i] == 0)
      document.write("AP");
     else
      document.write( "Infrastructure Client");
     document.write('</td> </tr> <tr> <th>SSID</th> <td class="show_space">'+rp_ssid[i] +'</td> </tr> <tr> <th><% multilang("208" "LANG_ENCRYPTION"); %></th> <td>'+rp_encrypt[i] +'</td> </tr> <tr> <th>BSSID</th> <td>'+rp_bssid[i] +'</td> </tr>');
     document.write('<tr >\n');
     if (rp_mode[i]==0 || rp_mode[i]==3) {
      document.write("<th><% multilang("157" "LANG_ASSOCIATED_CLIENTS"); %></th>\n");
      document.write("<td>"+rp_clientnum[i]+"</td></tr>");
     }
     else {
      document.write("<th><% multilang("261" "LANG_STATE"); %></th>\n");
