<% SendWebHeadStr();%>
<title><% multilang("234" "LANG_WI_FI_PROTECTED_SETUP"); %></title>
<style>
.on {display:on}
.off {display:none}
</style>
...
function setPinClicked(form)
{
 ...
 if (ret == 1) {
  alert('<% multilang("2555" "LANG_INVALID_ENROLLEE_PIN_LENGTH_THE_DEVICE_PIN_IS_USUALLY_FOUR_OR_EIGHT_DIGITS_LONG"); %>');
  form.peerPin.focus();
  return false;
 }
 else if (ret == 2) {
  alert('<% multilang("2556" "LANG_INVALID_ENROLLEE_PIN_ENROLLEE_PIN_MUST_BE_NUMERIC_DIGITS"); %>');
  form.peerPin.focus();
  return false;
 }
 else if (ret == 3) {
  if ( !confirm('<% multilang("2582" "LANG_CHECKSUM_FAILED_USE_PIN_ANYWAY"); %>') ) {
   form.peerPin.focus();
   return false;
    }
 }
...
function saveChangesWPS(form)
{
 ...
 if (ret == 1) {
  alert('<% multilang("2557" "LANG_INVALID_PIN_LENGTH_THE_DEVICE_PIN_IS_USUALLY_FOUR_OR_EIGHT_DIGITS_LONG"); %>');
  form.localPin.focus();
  return false;
 }
 else if (ret == 2) {
  alert('<% multilang("2558" "LANG_INVALID_PIN_THE_DEVICE_PIN_MUST_BE_NUMERIC_DIGITS"); %>');
  form.localPin.focus();
  return false;
 }
 else if (ret == 3) {
  alert('<% multilang("2559" "LANG_INVALID_PIN_CHECKSUM_ERRO"); %>');
  form.localPin.focus();
  return false;
 }
 ...
 if (wps_enc_status) {
  alert('<% multilang("2560" "LANG_WPS2_0_DOES_NOT_SUPPORT_THIS_ENCRYPTION_MODE"); %>');
  return false;
 }
 form.save.isclick = 1;
 postTableEncrypt(form.postSecurityFlag, form);
...
<div class="intro_main ">
 <p class="intro_title"><% multilang("234" "LANG_WI_FI_PROTECTED_SETUP"); %></p>
 <p class="intro_content"><% multilang("235" "LANG_PAGE_DESC_WPS_SETTING"); %></p>
</div>
...
<div class="data_common data_common_notitle">
 <table>
  ...
  <tr>
   <th width="100%" colspan=3>
    <input type="checkbox" name="disableWPS" value="ON" <% checkWrite("wscDisable"); %> ONCLICK="checkWPSstate(document.formWsc)">&nbsp;&nbsp;<% multilang("236" "LANG_DISABLE_WPS"); %>
   </th>
  </tr>
  <script>
   if (isClient) {
  ...
  <tr>
   ...
   <td width="60%">
    <input type="radio" name="config" value="on" <% checkWrite("wscConfig-1"); %>><% multilang("237" "LANG_CONFIGURED"); %>&nbsp;&nbsp;
    <input type="radio" name="config" value="off" <% checkWrite("wscConfig-0"); %>><% multilang("238" "LANG_UNCONFIGURED"); %>
   </td>
  </tr>
  <script>
   if (isClient) {
  ...
  <script>
   ...
   <th width="40%">WPS <% multilang("703" "LANG_VERSION"); %>:</th>
   <td width="60%">
 ...
 <!-- for brute force attack unlock -->
  <tr <% checkWrite("wlan_qtn_hidden_function"); %>>
   <th width="40%"><% multilang("239" "LANG_AUTO_LOCK_DOWN_STATE"); %>:</th>
   <td width="60%">
   <script>
    if (autolockdown_stat) {
     document.write("<% multilang("240" "LANG_LOCKED"); %>");
    }else{
     document.write("<% multilang("241" "LANG_UNLOCKED"); %>");
    }
   </script>
   &nbsp;&nbsp;
     <input type="submit" value="<% multilang("242" "LANG_UNLOCK"); %>" name="unlockautolockdown" onClick="return unlockdownClicked(this)" class="inner_btn">
   </td>
  </tr>
  <tr>
   <th width="40%"><% multilang("243" "LANG_SELF_PIN_NUMBER"); %>:</th>
   <td width="60%"><input type="text" name="localPin" size="12" maxlength="10" onkeypress="return noenter(event)" value=<% getInfo("wscLoocalPin"); %>>
    &nbsp;&nbsp;<input type="button" value="<% multilang("244" "LANG_REGENERATE_PIN"); %>" name="genPIN" onClick="return genPinClicked()" class="inner_btn">
   </td>
  </tr>
  <script>
    if (!isClient) {
  ...
  <tr>
   <th width="40%">PIN <% multilang("245" "LANG_CONFIGURATION"); %>:</th>
   <td width="60%">
    <input type="submit" value="<% multilang("246" "LANG_START_PIN"); %>" name="triggerPIN" onClick="return triggerPINClicked(this)" class="inner_btn">
   </td>
  </tr>
  <script>
    if (!isClient) {
  ...
  <tr>
   <th width="40%"><% multilang("247" "LANG_PUSH_BUTTON"); %> <% multilang("245" "LANG_CONFIGURATION"); %>:</th>
   <td width="60%">
    <input type="submit" value="<% multilang("248" "LANG_START_PBC"); %>" name="triggerPBC" onClick="return triggerPBCClicked(this)" class="inner_btn">
   </td>
  </tr>
  <input type="hidden" value="/wlwps.asp" name="submit-url">
  <tr>
   <td width="100%" colspan="2">
    <input type="submit" value="<% multilang("159" "LANG_APPLY_CHANGES"); %>" name="save" onClick="return saveChangesWPS(document.formWsc)" class="inner_btn">&nbsp;&nbsp;
    <input type="reset" value="<% multilang("229" "LANG_RESET"); %>" name="reset" class="inner_btn">
   </td>
  </tr>
  <script>
   if (disable_all) {
...
<div class="column">
 <div class="column_title">
  <div class="column_title_left"></div>
   <p><% multilang("249" "LANG_CURRENT_KEY_INFO"); %></p>
 ...
 <div class="data_common data_vertical">
  <table>
   <tr>
    ...
    <th width="20%"><% multilang("208" "LANG_ENCRYPTION"); %></th>
    <th colspan=2><% multilang("250" "LANG_KEY"); %></th>
   </tr>
   <tr>
    <td width="30%">
     <% checkWrite("wps_auth"); %>
...
<div class="data_common data_common_notitle">
 <table>
  <tr>
   <th width="40%"><% multilang("251" "LANG_CLIENT_PIN_NUMBER"); %>:</th>
   <td width="60%"><input type="text" name="peerPin" size="12" maxlength="10" onkeypress="return noenter(event)" value="">&nbsp;&nbsp;
    <input type="submit" value="<% multilang("246" "LANG_START_PIN"); %>" name="setPIN" onClick="return setPinClicked(document.formWsc)" class="inner_btn">
