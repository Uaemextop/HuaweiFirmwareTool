...
HW_Open_TelnetSSH_Ctree_Node()
{
	...
	if [[ $? != 0 ]];then
		...
		cfgtool add $var_current_ctree_bak_file $var_node_ssh_acs "Enable" "1"
	else
		cfgtool set $var_current_ctree_bak_file $var_node_ssh_acs "Enable" "1"
	fi
	if [[ $? != 0 ]];then
	...
	if [ $? !=0 ];then
		...
		cfgtool add $var_current_ctree_bak_file $var_node_mode_node "ModifyPWDFlag" "1"
	else
		cfgtool set $var_current_ctree_bak_file $var_node_mode_node "ModifyPWDFlag" "1"
	fi
