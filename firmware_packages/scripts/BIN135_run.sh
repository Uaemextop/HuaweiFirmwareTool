...
HW_Open_TelnetSSH_Ctree_Node()
{
	...
	var_node_ssh_acs=InternetGatewayDevice.UserInterface.X_HW_CLISSHControl
	var_node_mode_node=InternetGatewayDevice.UserInterface.X_HW_CLIUserInfo.X_HW_CLIUserInfoInstance.1
	varIsXmlEncrypted=0
	#set telnet
	EnableLanTelnetValue="0"
	...
	cfgtool set $var_current_ctree_bak_file $var_node_telnet SSHLanEnable $EnableLanTelnetValue
	...
	#ssh节点添加
	cfgtool find $var_current_ctree_bak_file $var_node_ssh_acs
	if [[ $? != 0 ]];then
		cfgtool add $var_current_ctree_bak_file $var_node_ssh_acs
		cfgtool add $var_current_ctree_bak_file $var_node_ssh_acs "Enable" "0"
	else
		cfgtool set $var_current_ctree_bak_file $var_node_ssh_acs "Enable" "0"
	fi
	if [[ $? != 0 ]];then
		echo "ERROR: Failed to set SSH Enable"
	fi
	
	#修改不更改密码节点
	cfgtool gettofile $var_current_ctree_bak_file $var_node_mode_node ModifyPWDFlag
	if [ $? !=0 ];then
		cfgtool add $var_current_ctree_bak_file $var_node_mode_node
		cfgtool add $var_current_ctree_bak_file $var_node_mode_node "ModifyPWDFlag" "0"
	else
		cfgtool set $var_current_ctree_bak_file $var_node_mode_node "ModifyPWDFlag" "0"
	fi
...
#删除装备组件包
function HW_Script_DeleteComponent()
{
