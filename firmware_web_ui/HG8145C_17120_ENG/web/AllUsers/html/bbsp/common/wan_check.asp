...
function getErrorMsg(fieldPrompt,errcode)
{
  ...
  var error=new Array("","MustBeInput","IsNumFirstChar","IsNum","VlanIDRange","MVlanIDRange");

  var errorCN=new Array("","VLAN必须输入","VLAN ID第一个字符不能为0","VLAN ID不合法，它只能是数字","VLAN ID 范围1~4094","VLAN ID 范围0~4094");
...
{	
	...
	if ((BirdgetoRoute() == true) && GetRouteWanCount() >= RouteWanNum)
	{
	 ...
	 if((Wan.Mode.toUpperCase().indexOf("BRIDGE") >= 0) && (Wan.ServiceList != 'INTERNET') && (Wan.ServiceList != 'IPTV') && (Wan.ServiceList != 'OTHER'))
	 {
		...
		{
			if(!(('TDE2' == CfgModeWord.toUpperCase()) && (Wan.ServiceList.toUpperCase().indexOf("INTERNET") >= 0) && (Wan.EncapMode.toUpperCase() == "PPPOE")))
			{
				AlertMsg("Wanlisterror");
				return false;
			}
	...
	{
		if((Wan.Mode.toUpperCase().indexOf("ROUTE") >= 0) && (Wan.ServiceList != 'TR069_INTERNET'))
		{
			AlertMsg("Wanlisterror2");
			return false;
		}
