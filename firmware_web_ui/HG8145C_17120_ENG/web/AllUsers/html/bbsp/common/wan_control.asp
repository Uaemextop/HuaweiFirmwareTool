...
function ControlUserName()
{
	...
	if (CfgModeWord.toUpperCase() == 'AIS' && curUserType != '0')
	{
		setDisable("UserName",1);
		setDisable("Password",1);
	} 
...
function ControlApplyButton()
{
	...
	if (!IsAdminUser())
	{
		...
		{
			DisableButton = false;	
			if (CfgModeWord.toUpperCase() == 'AIS')
			{
                DisableButton = true;			    
			}
		}
...
function ControlEditMode()
{
	...
	{
		if(Wan.Mode.toUpperCase() == "IP_ROUTED")
		{
...
{        
    if((true == IsAdminUser()) || (true == IsLanUpport()))
    {
        return;
    }

    if ((CfgModeWord.toUpperCase() == 'ANTEL' || "1" == "<%HW_WEB_GetFeatureSupport(FT_NORMAL_ONLY_SHOW_PPPOE_INFO);%>")
...
{       
	if (CfgModeWord.toUpperCase() != 'CLARO')
	{
	    return ;
	}
