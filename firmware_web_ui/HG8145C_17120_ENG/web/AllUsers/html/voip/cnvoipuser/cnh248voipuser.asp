...
function stProfile(Domain, DTMFMethod, X_HW_PortName)
{
    this.Domain = Domain;
    this.DTMFMethod = DTMFMethod;
	this.X_HW_PortName = X_HW_PortName;
}
var Profiles = <%HW_WEB_GetParaArryByDomain(InternetGatewayDevice.Services.VoiceService.1.VoiceProfile.1,DTMFMethod|X_HW_PortName,stProfile);%>;
 ...
 <script language="JavaScript" type="text/javascript">
            ...
            else
            {
                for (var i = 0; i < Line.length; i++)
                {
					...
					{
						html +='<td class="align_left" style="word-wrap:break-word; overflow:hidden;">' + Line[i].LineName + '&nbsp;</td>';
					}                   
                    html += '<td class="align_left" style="word-wrap:break-word; overflow:hidden;">' + Line[i].PhyReferenceList + '&nbsp;</td>';
                    html += '</tr>'; 
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" >
           ...
           <td class="width_per75 table_right" colspan="10">
			...
			<span class="gray"><script>document.write(h248user['vspa_gDspTmpNamehint']);</script></span>
           </td>
           </tr>
