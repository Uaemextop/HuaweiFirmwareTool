...
function UserControlPaser(_Id, _RealType, _DescRef, _RemarkRef, _ErrorMsgRef, _Require, _BindField, _InitValue, _TitleRef, _ElementClass, _MaxLength, _Textwidth, _ClickFuncApp, _disabled, _Class, _LanguageList)
{
	...
	this.BuildTextBox = function()
	{
		...
		return '<input id="'+this.Id+'" type="'+type+'" title="'+((this.Title.length > 0) ? this.Title:RemarkInfo.replace("(","").replace(")",""))+'"' + classstr + ' maxlength="'+this.MaxLength+'" RealType="'+this.RealType+'" BindField="'+this.BindField+'"' + DisabledStr + onclickStr + '/><font color="red" ' + RequireContentId + '>'+RequireContent+'</font><span class="gray" id="'+this.Id+'Remark">'+RemarkInfo+'</span>';
	}
	...
	this.BuildCheckBoxList = function()
	{
		...
		for (var i = 0; i < InitValueListLength; i++)
		{
			this.Remark = (this.Remark == null || this.Remark == undefined) ? "" : this.Remark;
			var tid = parseInt(i+1).toString();
			html += "<span " + DirStr + "id=\"Div"+this.Id+tid+"\" style=\"width:10pt\"><input " + classstr +" name=\""+this.Id+"\" id=\""+this.Id+tid+"\" type=\"checkbox\" value=\""+this.InitValueList[i].Value+"\"  RealType=\""+this.RealType+"\" BindField=\""+this.BindField+"\"/>"+this.InitValueList[i].Text+"<font color=\"red\" " +RequireContentId +">"+RequireContent+"</font><span class=\"gray\">"+this.Remark+"</span><\/span>  ";
	...
	this.BuildTextOtherBox = function()
	{
		...
		var classstr = GetElementClassStr(this.ElementClass, "TextBox");
		var html =  "<input id=\""+this.Id+"\" type=\""+type+"\"" + Titlestr + classstr + " maxlength=\""+this.MaxLength+"\" RealType=\""+this.RealType+"\" BindField=\""+this.BindField+"\"" + onclickStr + DisabledStr + "/><font color=\"red\" " + RequireContentId + ">" +RequireContent+"</font><span class=\"gray\" id=\""+this.Id+"Remark\">"+RemarkInfo+"</span>";
	...
	this.BuildSmartBoxList = function()
	{
		...
		var html =  BaseStart + "id=\""+baseTypeId+"\" type=\""+type+"\"" + Titlestr + classstr + " maxlength=\""+this.MaxLength+"\"  textwidth=\""+textwidth+"\" RealType=\""+this.RealType+"\" BindField=\""+this.BindField+"\"" + onclickStr + DisabledStr + BaseEndStr + "<font color=\"red\" " + RequireContentId + ">" +RequireContent+"</font><span " + baseremarkcssStr + " id=\""+this.Id+"Remark\">"+RemarkInfo+"</span>";
	...
	this.BuildTextDivbox = function()
	{
		...
		var type = (this.Id.toUpperCase().indexOf("PASSWORD") >= 0) ? "password" : "text";
		var RemarkInfo = (this.Remark == null || this.Remark == undefined) ? "" : this.Remark;
		var html =  "<input id=\""+this.Id+"\" type=\""+type+"\" title=\""+((this.Title.length > 0) ? this.Title:RemarkInfo.replace("(","").replace(")",""))+"\" class=\"TextBox\" maxlength=\""+this.MaxLength+"\" RealType=\""+this.RealType+"\" BindField=\""+this.BindField+"\"" + DisabledStr + "/><font color=\"red\" " + RequireContentId + ">"+RequireContent+"</font><span class=\"gray\" id=\""+this.Id+"Remark\">"+RemarkInfo+"</span>";
